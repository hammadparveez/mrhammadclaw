#!/bin/bash
# Setup MrHammadClaw Auth Management System
...
echo "=== MrHammadClaw Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'mrhammadclaw-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  MrHammadClaw message: Send warning via MrHammadClaw itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/mrhammadclaw-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/mrhammadclaw-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/mrhammadclaw-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now mrhammadclaw-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status mrhammadclaw-auth-monitor.timer"
echo ""
echo "Quick commands:"
