...
# Build and bundle MrHammadClaw into a minimal .app we can open.
# Outputs to dist/MrHammadClaw.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/MrHammadClaw.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="MrHammadClaw"
BUNDLE_ID="${BUNDLE_ID:-ai.mrhammadclaw.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/mrhammadclaw/mrhammadclaw/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/MrHammadClaw/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :MrHammadClawBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :MrHammadClawGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/MrHammadClaw"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/MrHammadClaw"
fi
chmod +x "$APP_ROOT/Contents/MacOS/MrHammadClaw"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/MrHammadClaw" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/MrHammadClaw/Resources/MrHammadClaw.icns" "$APP_ROOT/Contents/Resources/MrHammadClaw.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/MrHammadClaw/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying MrHammadClawKit resources"
MRHAMMADCLAWKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/MrHammadClawKit_MrHammadClawKit.bundle"
if [ -d "$MRHAMMADCLAWKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/MrHammadClawKit_MrHammadClawKit.bundle"
  cp -R "$MRHAMMADCLAWKIT_BUNDLE" "$APP_ROOT/Contents/Resources/MrHammadClawKit_MrHammadClawKit.bundle"
else
  echo "WARN: MrHammadClawKit resource bundle not found at $MRHAMMADCLAWKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running MrHammadClaw"
killall -q MrHammadClaw 2>/dev/null || true
