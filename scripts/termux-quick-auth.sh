...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/mrhammadclaw/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "MrHammadClaw Re-Auth" -c "After getting key, run: ssh $SERVER '~/mrhammadclaw/scripts/mobile-reauth.sh'" --id mrhammadclaw-auth
        ;;
    *)
