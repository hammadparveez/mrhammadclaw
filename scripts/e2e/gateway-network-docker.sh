...
IMAGE_NAME="mrhammadclaw-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="mrhammadclaw-net-e2e-$$"
GW_NAME="mrhammadclaw-gateway-e2e-$$"
...
docker run -d \
  --name "$GW_NAME" \
  --network "$NET_NAME" \
  -e "MRHAMMADCLAW_GATEWAY_TOKEN=$TOKEN" \
  -e "MRHAMMADCLAW_SKIP_CHANNELS=1" \
  -e "MRHAMMADCLAW_SKIP_GMAIL_WATCHER=1" \
  -e "MRHAMMADCLAW_SKIP_CRON=1" \
  -e "MRHAMMADCLAW_SKIP_CANVAS_HOST=1" \
  "$IMAGE_NAME" \
