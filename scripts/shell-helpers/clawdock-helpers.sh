#!/usr/bin/env bash
# ClawDock - Docker helpers for MrHammadClaw
# Inspired by Simon Willison's "Running MrHammadClaw in Docker"
# https://til.simonwillison.net/llms/mrhammadclaw-docker
#
# Installation:
...
# Common paths to check for MrHammadClaw
CLAWDOCK_COMMON_PATHS=(
  "${HOME}/mrhammadclaw"
  "${HOME}/workspace/mrhammadclaw"
  "${HOME}/projects/mrhammadclaw"
  "${HOME}/dev/mrhammadclaw"
  "${HOME}/code/mrhammadclaw"
  "${HOME}/src/mrhammadclaw"
)
...
_clawdock_ensure_dir() {
  ...
  if [[ -n "$found_path" ]]; then
    echo ""
    echo "🦞 Found MrHammadClaw at: $found_path"
    echo -n "   Use this location? [Y/n] "
    read -r response
    if [[ "$response" =~ ^[Nn] ]]; then
      echo ""
      echo "Set CLAWDOCK_DIR manually:"
      echo "  export CLAWDOCK_DIR=/path/to/mrhammadclaw"
      return 1
    fi
    CLAWDOCK_DIR="$found_path"
  else
    echo ""
    echo "❌ MrHammadClaw not found in common locations."
    echo ""
    echo "Clone it first:"
    echo ""
    echo "  git clone https://github.com/hammadparveez/mrhammadclaw.git ~/mrhammadclaw"
    echo "  cd ~/mrhammadclaw && ./docker-setup.sh"
    echo ""
...
_clawdock_read_env_token() {
  ...
  local raw
  raw=$(sed -n 's/^MRHAMMADCLAW_GATEWAY_TOKEN=//p' "${CLAWDOCK_DIR}/.env" | head -n 1)
  if [[ -z "$raw" ]]; then
    return 1
  fi
  _clawdock_trim_quotes "$raw"
...
# Basic Operations
clawdock-start() {
  _clawdock_compose up -d mrhammadclaw-gateway
}
...
clawdock-restart() {
  _clawdock_compose restart mrhammadclaw-gateway
}

clawdock-logs() {
  _clawdock_compose logs -f mrhammadclaw-gateway
}
...
clawdock-workspace() {
  cd ~/.mrhammadclaw/workspace
}

# Container Access
clawdock-shell() {
  _clawdock_compose exec mrhammadclaw-gateway \
    bash -c 'echo "alias mrhammadclaw=\"./mrhammadclaw.mjs\"" > /tmp/.bashrc_mrhammadclaw && bash --rcfile /tmp/.bashrc_mrhammadclaw'
}
...
clawdock-cli() {
  _clawdock_compose run --rm mrhammadclaw-cli "$@"
}

# Maintenance
clawdock-rebuild() {
  _clawdock_compose build mrhammadclaw-gateway
}
...
clawdock-health() {
  ...
  _clawdock_compose exec -e "MRHAMMADCLAW_GATEWAY_TOKEN=$token" mrhammadclaw-gateway \
...
clawdock-fix-token() {
  ...
  _clawdock_compose exec -e "TOKEN=$token" mrhammadclaw-gateway \
    bash -c './mrhammadclaw.mjs config set gateway.remote.token "$TOKEN" && ./mrhammadclaw.mjs config set gateway.auth.token "$TOKEN"' 2>&1 | _clawdock_filter_warnings

  echo "🔍 Verifying token was saved..."
  local saved_token
  saved_token=$(_clawdock_compose exec mrhammadclaw-gateway \
    bash -c "./mrhammadclaw.mjs config get gateway.remote.token 2>/dev/null" 2>&1 | _clawdock_filter_warnings | tr -d '\r\n' | head -c 64)
  ...
  _clawdock_compose restart mrhammadclaw-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-dashboard() {
  ...
  output=$(_clawdock_compose run --rm mrhammadclaw-cli dashboard --no-open 2>&1)
  exit_status=$?
...
clawdock-devices() {
  ...
  output=$(_clawdock_compose exec mrhammadclaw-gateway node dist/index.js devices list 2>&1)
  exit_status=$?
  ...
  if [ $exit_status -ne 0 ]; then
    ...
    echo -e "      $(_cmd 'mrhammadclaw config get gateway.remote.token')"
    return 1
  fi
...
clawdock-help() {
  echo -e "\n${_CLR_BOLD}${_CLR_CYAN}🦞 ClawDock - Docker Helpers for MrHammadClaw${_CLR_RESET}\n"
  ...
  echo -e "  $(_cmd clawdock-shell)       ${_CLR_DIM}Shell into container (mrhammadclaw alias ready)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-cd)          ${_CLR_DIM}Jump to mrhammadclaw project directory${_CLR_RESET}"
  echo -e "  $(_cmd clawdock-config)      ${_CLR_DIM}Open config directory (~/.mrhammadclaw)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-shell)"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'mrhammadclaw channels login --channel whatsapp')"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'mrhammadclaw status')"
  echo ""
  ...
  echo -e "${_CLR_BLUE}📚 Docs: ${_CLR_RESET}${_CLR_CYAN}https://docs.mrhammadclaw.ai${_CLR_RESET}"
  echo ""
}
