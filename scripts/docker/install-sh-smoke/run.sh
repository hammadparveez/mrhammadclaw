...
INSTALL_URL="${MRHAMMADCLAW_INSTALL_URL:-https://mrhammadclaw.bot/install.sh}"
SMOKE_PREVIOUS_VERSION="${MRHAMMADCLAW_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${MRHAMMADCLAW_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="mrhammadclaw"
PACKAGE_NAME="${MRHAMMADCLAW_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (MRHAMMADCLAW_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${MRHAMMADCLAW_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${MRHAMMADCLAW_INSTALL_LATEST_OUT:-}"
fi
if [[ -n "$CMD_PATH" ]]; then
