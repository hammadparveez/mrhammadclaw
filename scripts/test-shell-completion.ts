 ...
 * `mrhammadclaw update`. Use it to verify the completion installation flow
...
\`mrhammadclaw update\`, \`mrhammadclaw doctor\`, and \`mrhammadclaw onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# MrHammadClaw Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`mrhammadclaw completion --install\` later to enable.`));
    return;
  }
