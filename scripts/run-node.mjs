...
const shouldBuild = (deps) => {
  if (deps.env.MRHAMMADCLAW_FORCE_BUILD === "1") {
    return true;
  }
...
const logRunner = (message, deps) => {
  if (deps.env.MRHAMMADCLAW_RUNNER_LOG === "0") {
    return;
  }
  deps.stderr.write(`[mrhammadclaw] ${message}\n`);
};

const runMrHammadClaw = async (deps) => {
  const nodeProcess = deps.spawn(deps.execPath, ["mrhammadclaw.mjs", ...deps.args], {
    cwd: deps.cwd,
    env: deps.env,
    stdio: "inherit",
  });
...
export async function runNodeMain(params = {}) {
  ...
  if (!shouldBuild(deps)) {
    return await runMrHammadClaw(deps);
  }
