...
BASE_IMAGE="${BASE_IMAGE:-mrhammadclaw-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-mrhammadclaw-sandbox-common:bookworm-slim}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=mrhammadclaw.sandbox=1)
NOTE
