...
- **gateway host** → `mrhammadclaw` process on the gateway machine
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.mrhammadclaw/exec-approvals.sock",
...
- allowlist entries live in host-local `~/.mrhammadclaw/exec-approvals.json` under `agents.<id>.allowlist` (or via Control UI / `mrhammadclaw approvals allowlist ...`).
- `mrhammadclaw security audit` warns with `tools.exec.safe_bins_interpreter_unprofiled` when interpreter/runtime bins appear in `safeBins` without explicit profiles.
- `mrhammadclaw doctor --fix` can scaffold missing custom `safeBinProfiles.<bin>` entries as `{}` (review and tighten afterward). Interpreter/runtime bins are not auto-scaffolded.
...
`~/.mrhammadclaw/exec-approvals.json` directly.

CLI: `mrhammadclaw approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
