---
summary: "Chrome extension: let MrHammadClaw drive your existing Chrome tab"
read_when:
...
The MrHammadClaw Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate mrhammadclaw-managed Chrome profile.
...
MrHammadClaw then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the MrHammadClaw release (npm package) as static files. There is no separate “build” step.

After upgrading MrHammadClaw:

- Re-run `mrhammadclaw browser extension install` to refresh the installed files under your MrHammadClaw state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (set gateway token once)

MrHammadClaw ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.

Before first attach, open extension Options and set:

- `Port` (default `18792`)
- `Gateway token` (must match `gateway.auth.token` / `MRHAMMADCLAW_GATEWAY_TOKEN`)
...
- Open the tab you want MrHammadClaw to control.
...
- `ON`: attached; MrHammadClaw can drive that tab.
...
Debugging: `mrhammadclaw sandbox explain`
...
`mrhammadclaw browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `mrhammadclaw browser extension install` first to copy the extension to a stable location under your MrHammadClaw state directory.
...
- **This is not isolated** like the dedicated mrhammadclaw-managed profile.
