...
- `gateway`/`node` approvals are controlled by `~/.mrhammadclaw/exec-approvals.json`.
...
- `host=sandbox`: runs `sh -lc` (login shell) inside the container, so `/etc/profile` may reset `PATH`.
  MrHammadClaw prepends `env.PATH` after profile sourcing via an internal env var (no shell interpolation);
...
`mrhammadclaw security audit` warns when interpreter/runtime `safeBins` entries are missing explicit profiles, and `mrhammadclaw doctor --fix` can scaffold missing custom `safeBinProfiles` entries.
