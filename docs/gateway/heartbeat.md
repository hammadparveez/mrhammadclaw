...
- During heartbeat runs, MrHammadClaw treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `mrhammadclaw sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, MrHammadClaw skips the heartbeat run entirely (no model call).
...
```bash
mrhammadclaw system event --text "Check for urgent follow-ups" --mode now
```
