...
`mrhammadclaw doctor` is the repair + migration tool for MrHammadClaw. It fixes stale
...
```bash
mrhammadclaw doctor --yes
```

Accept defaults without prompting (including restart/service/sandbox repair steps when applicable).

```bash
mrhammadclaw doctor --repair
```

Apply recommended repairs without prompting (repairs + restarts where safe).

```bash
mrhammadclaw doctor --repair --force
```
...
```bash
cat ~/.mrhammadclaw/mrhammadclaw.json
```
...
- Extra workspace dir detection (`~/mrhammadclaw`).
...
you to run `mrhammadclaw doctor`.

Doctor will:

- Explain which legacy keys were found.
- Show the migration it applied.
- Rewrite `~/.mrhammadclaw/mrhammadclaw.json` with the updated schema.
...
- Sessions store + transcripts:
  - from `~/.mrhammadclaw/sessions/` to `~/.mrhammadclaw/agents/<agentId>/sessions/`
- Agent dir:
  - from `~/.mrhammadclaw/agent/` to `~/.mrhammadclaw/agents/<agentId>/agent/`
- WhatsApp auth state (Baileys):
  - from legacy `~/.mrhammadclaw/credentials/*.json` (except `oauth.json`)
  - to `~/.mrhammadclaw/credentials/whatsapp/<accountId>/...` (default account id: `default`)

These migrations are best-effort and idempotent; doctor will emit warnings when
it leaves any legacy folders behind as backups. The Gateway/CLI also auto-migrates
the legacy sessions + agent dir on startup so history/auth/models land in the
per-agent path without a manual doctor run. WhatsApp auth is intentionally only
migrated via `mrhammadclaw doctor`.
...
- **Multiple state dirs**: warns when multiple `~/.mrhammadclaw` folders exist across
  home directories or when `MRHAMMADCLAW_STATE_DIR` points elsewhere (history can
  split between installs).
- **Remote mode reminder**: if `gateway.mode=remote`, doctor reminds you to run
  it on the remote host (the state lives there).
- **Config file permissions**: warns if `~/.mrhammadclaw/mrhammadclaw.json` is
...
offers to remove them and install the MrHammadClaw service using the current gateway
port. It can also scan for extra gateway-like services and print cleanup hints.
Profile-named MrHammadClaw gateway services are considered first-class and are not
flagged as "extra."
...
generate a token. Use `mrhammadclaw doctor --generate-gateway-token` to force token
...
- `mrhammadclaw doctor` prompts before rewriting supervisor config.
- `mrhammadclaw doctor --yes` accepts the default repair prompts.
- `mrhammadclaw doctor --repair` applies recommended fixes without prompts.
- `mrhammadclaw doctor --repair --force` overwrites custom supervisor configs.
- You can always force a full rewrite via `mrhammadclaw gateway install --force`.
