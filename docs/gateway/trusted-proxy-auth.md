...
read_when:
  - Running MrHammadClaw behind an identity-aware proxy
  - Setting up Pomerium, Caddy, or nginx with OAuth in front of MrHammadClaw
...
- You run MrHammadClaw behind an **identity-aware proxy** (Pomerium, Caddy + OAuth, nginx + oauth2-proxy, Traefik + forward auth)
...
3. MrHammadClaw checks that the request came from a **trusted proxy IP** (configured in `gateway.trustedProxies`)
4. MrHammadClaw extracts the user identity from the configured header
...
- MrHammadClaw can stay on loopback HTTP behind the proxy.
...
If MrHammadClaw itself serves HTTPS directly (no TLS-terminating proxy), set:
...
```yaml
routes:
  - from: https://mrhammadclaw.example.com
    to: http://mrhammadclaw-gateway:18789
    policy:
      - allow:
          or:
            - email:
...
```
mrhammadclaw.example.com {
    authenticate with oauth2_provider
    authorize with policy1

    reverse_proxy mrhammadclaw:18789 {
...
location / {
    ...
    proxy_pass http://mrhammadclaw:18789;
...
`mrhammadclaw security audit` will flag trusted-proxy auth with a **critical** severity finding. This is intentional — it's a reminder that you're delegating security to your proxy setup.
...
3. Update MrHammadClaw config with trusted-proxy auth
4. Restart the Gateway
5. Test WebSocket connections from the Control UI
6. Run `mrhammadclaw security audit` and review findings
