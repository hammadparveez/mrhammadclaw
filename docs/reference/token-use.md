---
summary: "How MrHammadClaw builds prompt context and reports token usage + costs"
read_when:
...
MrHammadClaw tracks **tokens**, not characters. Tokens are model-specific, but most
OpenAI-style models average ~4 characters per token for English text.

## How the system prompt is built

MrHammadClaw assembles its own system prompt on every run. It includes:
...
For images, MrHammadClaw downscales transcript/tool image payloads before provider calls.
...
- `/usage cost` → shows a local cost summary from MrHammadClaw session logs.

Other surfaces:

- **TUI/Web TUI:** `/status` + `/usage` are supported.
- **CLI:** `mrhammadclaw status --usage` and `mrhammadclaw channels list` show
...
`cacheWrite`. If pricing is missing, MrHammadClaw shows tokens only. OAuth tokens
never show dollar cost.

## Cache TTL and pruning impact

Provider prompt caching only applies within the cache TTL window. MrHammadClaw can
...
Anthropic's 1M context window is currently beta-gated. MrHammadClaw can inject the
...
MrHammadClaw skips the `context-1m-*` beta header because Anthropic currently
