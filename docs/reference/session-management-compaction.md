...
This document explains how MrHammadClaw manages sessions end-to-end:
...
MrHammadClaw is designed around a single **Gateway process** that owns session state.
...
MrHammadClaw persists sessions in two layers:
...
- Transcripts: `~/.mrhammadclaw/agents/<agentId>/sessions/<sessionId>.jsonl`
  - Telegram topic sessions: `.../<sessionId>-topic-<threadId>.jsonl`

MrHammadClaw resolves these via `src/config/sessions.ts`.
...
In `mode: "warn"`, MrHammadClaw reports potential evictions but does not mutate the store/files.
...
- `cron.runLog.maxBytes` + `cron.runLog.keepLines` prune `~/.mrhammadclaw/cron/runs/<jobId>.jsonl` files (defaults: `2_000_000` bytes and `2000` lines).
...
MrHammadClaw intentionally does **not** “fix up” transcripts; the Gateway uses `SessionManager` to read/write them.
...
These are Pi runtime semantics (MrHammadClaw consumes the events, but Pi decides when to compact).
...
MrHammadClaw also enforces a safety floor for embedded runs:

- If `compaction.reserveTokens < reserveTokensFloor`, MrHammadClaw bumps it.
- Default floor is `20000` tokens.
- Set `agents.defaults.compaction.reserveTokensFloor: 0` to disable the floor.
- If it’s already higher, MrHammadClaw leaves it alone.
...
- `mrhammadclaw status` (CLI)
- `mrhammadclaw sessions` / `sessions --json`
...
MrHammadClaw supports “silent” turns for background tasks where the user should not see intermediate output.

Convention:

- The assistant starts its output with `NO_REPLY` to indicate “do not deliver a reply to the user”.
- MrHammadClaw strips/suppresses this in the delivery layer.

As of `2026.1.10`, MrHammadClaw also suppresses **draft/typing streaming** when a partial chunk begins with `NO_REPLY`, so silent operations don’t leak partial output mid-turn.
...
MrHammadClaw uses the **pre-threshold flush** approach:
...
Pi also exposes a `session_before_compact` hook in the extension API, but MrHammadClaw’s
...
- Store vs transcript mismatch? Confirm the Gateway host and the store path from `mrhammadclaw status`.
