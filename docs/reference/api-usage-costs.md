...
MrHammadClaw features that can generate provider usage or paid API calls.
...
- `mrhammadclaw status --usage` and `mrhammadclaw channels list` show provider **usage windows**
...
MrHammadClaw can pick up credentials from:
...
- `mrhammadclaw models status --json`
...
`mrhammadclaw models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
