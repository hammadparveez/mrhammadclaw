...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in MrHammadClaw
...
Running MrHammadClaw via Node with `tsx` fails at startup with:

```
[mrhammadclaw] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `mrhammadclaw status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch mrhammadclaw.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node mrhammadclaw.mjs status` works on Node 25.
