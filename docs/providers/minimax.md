---
summary: "Use MiniMax M2.1 in MrHammadClaw"
read_when:
  - You want MiniMax models in MrHammadClaw
...
```bash
mrhammadclaw plugins enable minimax-portal-auth  # skip if already loaded.
mrhammadclaw gateway restart  # restart if gateway is already running
mrhammadclaw onboard --auth-choice minimax-portal
```
...
See [MiniMax OAuth plugin README](https://github.com/hammadparveez/mrhammadclaw/tree/main/extensions/minimax-portal-auth) for details.
...
Configure manually via `mrhammadclaw.json`:
...
- Use `mrhammadclaw models list` and `mrhammadclaw models set minimax/MiniMax-M2.1` to switch.
...
- Running `mrhammadclaw configure` and selecting **MiniMax M2.1**, or
