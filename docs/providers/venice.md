---
summary: "Use Venice AI privacy-focused models in MrHammadClaw"
read_when:
  - You want privacy-focused inference in MrHammadClaw
...
```bash
mrhammadclaw onboard --auth-choice venice-api-key
```
...
```bash
mrhammadclaw agent --model venice/llama-3.3-70b --message "Hello, are you working?"
```

## Model Selection

After setup, MrHammadClaw shows all available Venice models. Pick based on your needs:
...
```bash
mrhammadclaw models set venice/claude-opus-45
mrhammadclaw models set venice/llama-3.3-70b
```

List all available models:

```bash
mrhammadclaw models list | grep venice
```

## Configure via `mrhammadclaw configure`

1. Run `mrhammadclaw configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
MrHammadClaw automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
mrhammadclaw agent --model venice/llama-3.3-70b --message "Quick health check"

# Use Claude via Venice (anonymized)
mrhammadclaw agent --model venice/claude-opus-45 --message "Summarize this task"

# Use uncensored model
mrhammadclaw agent --model venice/venice-uncensored --message "Draft options"

# Use vision model with image
mrhammadclaw agent --model venice/qwen3-vl-235b-a22b --message "Review attached image"

# Use coding model
mrhammadclaw agent --model venice/qwen3-coder-480b-a35b-instruct --message "Refactor this function"
```
...
The Venice model catalog updates dynamically. Run `mrhammadclaw models list` to see currently available models. Some models may be temporarily offline.
