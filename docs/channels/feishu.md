...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects MrHammadClaw to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.
...
```bash
mrhammadclaw plugins install @mrhammadclaw/feishu
```

Local checkout (when running from a git repo):

```bash
mrhammadclaw plugins install ./extensions/feishu
```
...
If you just installed MrHammadClaw, run the wizard:
...
2. Configuring app credentials in MrHammadClaw
...
1. You already ran `mrhammadclaw channels add` for Feishu
2. The gateway is running (`mrhammadclaw gateway status`)
...
Edit `~/.mrhammadclaw/mrhammadclaw.json`:
...
```bash
mrhammadclaw pairing approve feishu <CODE>
```
...
- **Approve pairing**:
  ...
  ```bash
  mrhammadclaw pairing list feishu
  mrhammadclaw pairing approve feishu <CODE>
  ```
...
2. Run `mrhammadclaw logs --follow` and look for `chat_id`
...
2. Run `mrhammadclaw logs --follow` and look for `open_id`
...
| Command                        | Description                   |
| ------------------------------ | ----------------------------- |
| `mrhammadclaw gateway status`  | Show gateway status           |
| `mrhammadclaw gateway install` | Install/start gateway service |
| `mrhammadclaw gateway stop`    | Stop gateway service          |
| `mrhammadclaw gateway restart` | Restart gateway service       |
| `mrhammadclaw logs --follow`   | Tail gateway logs             |
...
4. Check logs: `mrhammadclaw logs --follow`
...
5. Ensure the gateway is running: `mrhammadclaw gateway status`
6. Check logs: `mrhammadclaw logs --follow`
  ...
  agents: {
    list: [
      { id: "main" },
      {
        ...
        agentDir: "/home/user/.mrhammadclaw/agents/clawd-fan/agent",
      },
      {
        id: "clawd-xi",
        workspace: "/home/user/clawd-xi",
        agentDir: "/home/user/.mrhammadclaw/agents/clawd-xi/agent",
      },
    ],
  },
  bindings: [
