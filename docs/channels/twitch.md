...
read_when:
  - Setting up Twitch chat integration for MrHammadClaw
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. MrHammadClaw connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
mrhammadclaw plugins install @mrhammadclaw/twitch
```

Local checkout (when running from a git repo):

```bash
mrhammadclaw plugins install ./extensions/twitch
```
...
4. Configure the token:
   - Env: `MRHAMMADCLAW_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "mrhammadclaw", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use MRHAMMADCLAW_TWITCH_ACCESS_TOKEN env var)
...
```bash
MRHAMMADCLAW_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
