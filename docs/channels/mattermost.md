---
summary: "Mattermost bot setup and MrHammadClaw config"
read_when:
...
```bash
mrhammadclaw plugins install @mrhammadclaw/mattermost
```

Local checkout (when running from a git repo):

```bash
mrhammadclaw plugins install ./extensions/mattermost
```
...
4. Configure MrHammadClaw and start the gateway.
...
- Approve via:
  - `mrhammadclaw pairing list mattermost`
  - `mrhammadclaw pairing approve mattermost <CODE>`
...
Use these target formats with `mrhammadclaw message send` or cron/webhooks:
