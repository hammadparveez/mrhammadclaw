...
Tlon is a decentralized messenger built on Urbit. MrHammadClaw connects to your Urbit ship and can
...
```bash
mrhammadclaw plugins install @mrhammadclaw/tlon
```

Local checkout (when running from a git repo):

```bash
mrhammadclaw plugins install ./extensions/tlon
```
...
By default, MrHammadClaw blocks private/internal hostnames and IP ranges for this plugin (SSRF hardening).
...
Use these with `mrhammadclaw message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, MrHammadClaw replies in-thread.
