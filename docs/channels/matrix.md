...
Matrix is an open, decentralized messaging protocol. MrHammadClaw connects as a Matrix **user**
...
```bash
mrhammadclaw plugins install @mrhammadclaw/matrix
```

Local checkout (when running from a git repo):

```bash
mrhammadclaw plugins install ./extensions/matrix
```
...
1. Install the Matrix plugin:
   - From npm: `mrhammadclaw plugins install @mrhammadclaw/matrix`
   - From a local checkout: `mrhammadclaw plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: MrHammadClaw calls the same
     login endpoint, stores the access token in `~/.mrhammadclaw/credentials/matrix/credentials.json`,
...
- On first connection, MrHammadClaw requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  MrHammadClaw logs a warning.
...
`~/.mrhammadclaw/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - `mrhammadclaw pairing list matrix`
  - `mrhammadclaw pairing approve matrix <CODE>`
...
- On startup, MrHammadClaw resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.
