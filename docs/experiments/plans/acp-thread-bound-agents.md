...
This plan defines how MrHammadClaw should support ACP coding agents in thread-capable channels (Discord first) with production-level lifecycle and recovery.
...
- MrHammadClaw core owns ACP control plane concerns
...
MrHammadClaw should not reimplement ACP transport internals in core.
MrHammadClaw should not rely on a pure plugin-only interception path for routing.

## North-star architecture (holy grail)

Treat ACP as a first-class control plane in MrHammadClaw, with pluggable runtime adapters.
...
- move ACP control-plane state to a dedicated SQLite store (WAL mode) under MrHammadClaw state dir
...
Core (must be in MrHammadClaw core):
...
- ACP event stream is normalized to MrHammadClaw reply chunks
...
For the first production backend (`extensions/acpx`), MrHammadClaw and acpx are
...
- encoded payload fields:
  - `name` (acpx named session; uses MrHammadClaw `sessionKey`)
...
- MrHammadClaw consumes ndjson events from `acpx --format json --json-strict`
