...
read_when:
  - Designing workspace memory (~/.mrhammadclaw/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep MrHammadClaw integration
...
Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/.mrhammadclaw/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.mrhammadclaw/workspace` as canonical human-readable memory.
...
```
~/.mrhammadclaw/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in MrHammadClaw**, but keep a separable core library.

### Why integrate into MrHammadClaw?

- MrHammadClaw already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `mrhammadclaw memory recall "…" --k 25 --since 30d`
  - `mrhammadclaw memory reflect --since 7d`
...
Pragmatic take for `~/.mrhammadclaw/workspace`:
