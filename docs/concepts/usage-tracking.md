...
- `/usage cost` in chats: local cost summary aggregated from MrHammadClaw session logs.
- CLI: `mrhammadclaw status --usage` prints a full per-provider breakdown.
- CLI: `mrhammadclaw channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
