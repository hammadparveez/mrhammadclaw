...
read_when:
  ...
  - Wiring external systems into MrHammadClaw
title: "Webhooks"
---
...
```json5
{
  hooks: {
    enabled: true,
    token: "${MRHAMMADCLAW_HOOKS_TOKEN}",
...
- `hooks.transformsDir` + `transform.module` loads a JS/TS module for custom logic.
  - `hooks.transformsDir` (if set) must stay within the transforms root under your MrHammadClaw config directory (typically `~/.mrhammadclaw/hooks/transforms`).
...
- `mrhammadclaw webhooks gmail setup` writes `hooks.gmail` config for `mrhammadclaw webhooks gmail run`.
