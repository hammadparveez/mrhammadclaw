...
- Jobs persist under `~/.mrhammadclaw/cron/` so restarts don’t lose schedules.
...
mrhammadclaw cron list
mrhammadclaw cron run <job-id>
mrhammadclaw cron runs --id <job-id>
```
...
Cron jobs are persisted on the Gateway host at `~/.mrhammadclaw/cron/jobs.json` by default.
The Gateway loads the file into memory and writes it back on changes, so manual edits
are only safe when the Gateway is stopped. Prefer `mrhammadclaw cron add/edit` or the cron
...
To reduce top-of-hour load spikes across many gateways, MrHammadClaw applies a
...
If `delivery` is omitted for isolated jobs, MrHammadClaw defaults to `announce`.
...
- Job store: `~/.mrhammadclaw/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.mrhammadclaw/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned by size and line count).
...
```json5
{
  cron: {
    ...
    store: "~/.mrhammadclaw/cron/jobs.json",
...
- For removed run sessions no longer referenced by the session store, MrHammadClaw archives transcript files and purges old deleted archives on the same retention window.
...
- review growth periodically with `mrhammadclaw cron runs` and adjust retention before logs become large
...
mrhammadclaw cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
mrhammadclaw cron edit <jobId> --agent ops
mrhammadclaw cron edit <jobId> --clear-agent
```

Manual run (force is the default, use `--due` to only run when due):

```bash
mrhammadclaw cron run <jobId>
mrhammadclaw cron run <jobId> --due
```

Edit an existing job (patch fields):

```bash
mrhammadclaw cron edit <jobId> \
...
```bash
mrhammadclaw cron edit <jobId> --exact
```
...
```bash
mrhammadclaw system event --mode now --text "Next heartbeat: check battery."
```
...
- `cron.run` (force or due), `cron.runs`
  For immediate system events without a job, use [`mrhammadclaw system event`](/cli/system).

## Troubleshooting

### “Nothing runs”

- Check cron is enabled: `cron.enabled` and `MRHAMMADCLAW_SKIP_CRON`.
- Check the Gateway is running continuously (cron runs inside the Gateway process).
- For `cron` schedules: confirm timezone (`--tz`) vs the host timezone.

### A recurring job keeps delaying after failures

- MrHammadClaw applies exponential retry backoff for recurring jobs after consecutive errors:
