...
- Publish as `mrhammadclaw/plugin-sdk` (or export from core under `mrhammadclaw/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)

Scope: everything that touches core runtime behavior.
Accessed via `MrHammadClawPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: MrHammadClawConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: MrHammadClawConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: MrHammadClawConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: MrHammadClawConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: MrHammadClawConfig): string;
  };
};
```
...
- Introduce `mrhammadclaw/plugin-sdk`.
- Add `api.runtime` to `MrHammadClawPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `mrhammadclawRuntime: ">=2026.2.0"`).
