...
service via the external `mrhammadclaw` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `ai.mrhammadclaw.gateway`
  (or `ai.mrhammadclaw.<profile>` when using `--profile`/`MRHAMMADCLAW_PROFILE`; legacy `com.mrhammadclaw.*` is supported).
