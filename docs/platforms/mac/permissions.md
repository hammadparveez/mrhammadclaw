...
- Same path: run the app from a fixed location (for MrHammadClaw, `dist/MrHammadClaw.app`).
...
```bash
sudo tccutil reset Accessibility ai.mrhammadclaw.mac
sudo tccutil reset ScreenCapture ai.mrhammadclaw.mac
...
Workaround: move files into the MrHammadClaw workspace (`~/.mrhammadclaw/workspace`) if you want to avoid per-folder grants.
