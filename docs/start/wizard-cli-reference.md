...
read_when:
  - You need detailed behavior for mrhammadclaw onboard
...
This page is the full reference for `mrhammadclaw onboard`.
...
<Steps>
  <Step title="Existing config detection">
    - If `~/.mrhammadclaw/mrhammadclaw.json` exists, choose Keep, Modify, or Reset.
    - Re-running the wizard does not wipe anything unless you explicitly choose Reset (or pass `--reset`).
    - If config is invalid or contains legacy keys, the wizard stops and asks you to run `mrhammadclaw doctor` before continuing.
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via
      `mrhammadclaw pairing approve <channel> <code>` or use allowlists.
  </Step>
  ...
  <Step title="Health check">
    - Starts gateway (if needed) and runs `mrhammadclaw health`.
    - `mrhammadclaw status --deep` adds gateway health probes to status output.
  </Step>
  <Step title="Skills">
...
- OAuth credentials: `~/.mrhammadclaw/credentials/oauth.json`
- Auth profiles (API keys + OAuth): `~/.mrhammadclaw/agents/<agentId>/agent/auth-profiles.json`
...
`~/.mrhammadclaw/credentials/oauth.json` (or `$MRHAMMADCLAW_STATE_DIR/credentials/oauth.json`)
to the gateway host.
</Note>
...
- Command reference: [`mrhammadclaw onboard`](/cli/onboard)
