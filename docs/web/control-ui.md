...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/mrhammadclaw`)
...
If the page fails to load, start the Gateway first: `mrhammadclaw gateway`.
...
you revoke it with `mrhammadclaw devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.mrhammadclaw/mrhammadclaw.json` (`config.get`, `config.set`)
...
- Stop:
  ...
  - Type `/stop` (or standalone abort phrases like `stop`, `stop action`, `stop run`, `stop mrhammadclaw`, `please stop`) to abort out-of-band
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. MrHammadClaw
...
MrHammadClaw **blocks** Control UI connections without device identity.
