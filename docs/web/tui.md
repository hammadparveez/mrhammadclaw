...
```bash
mrhammadclaw tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `mrhammadclaw tui --deliver`
...
- Check the Gateway logs: `mrhammadclaw logs --follow`.
- Confirm the agent can run: `mrhammadclaw status` and `mrhammadclaw models status`.
...
- No agents in picker: check `mrhammadclaw agents list` and your routing config.
