...
- `~/Library/Application Support/MrHammadClaw/canvas/<session>/...`

Canvas 面板通过**自定义 URL 方案**提供这些文件：

- `mrhammadclaw-canvas://<session>/<path>`

示例：

- `mrhammadclaw-canvas://main/` → `<canvasRoot>/main/index.html`
- `mrhammadclaw-canvas://main/assets/app.css` → `<canvasRoot>/main/assets/app.css`
- `mrhammadclaw-canvas://main/widgets/todo/` → `<canvasRoot>/main/widgets/todo/index.html`
...
```bash
mrhammadclaw nodes canvas present --node <id>
mrhammadclaw nodes canvas navigate --node <id> --url "/"
mrhammadclaw nodes canvas eval --node <id> --js "document.title"
mrhammadclaw nodes canvas snapshot --node <id>
```
...
```
http://<gateway-host>:18793/__mrhammadclaw__/a2ui/
```
...
mrhammadclaw nodes canvas a2ui push --jsonl /tmp/a2ui-v0.8.jsonl --node <id>
```

快速测试：

```bash
mrhammadclaw nodes canvas a2ui push --node <id> --text "Hello from A2UI"
```
...
```js
window.location.href = "mrhammadclaw://agent?message=Review%20this%20design";
```
