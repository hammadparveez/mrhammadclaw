...
class InvokeDispatcher(
  ...
  suspend fun handleInvoke(command: String, paramsJson: String?): GatewaySession.InvokeResult {
    ...
    return when (command) {
      ...
      MrHammadClawCanvasCommand.Hide.rawValue -> GatewaySession.InvokeResult.ok(null)
      MrHammadClawCanvasCommand.Navigate.rawValue -> {
        val url = CanvasController.parseNavigateUrl(paramsJson)
        canvas.navigate(url)
        GatewaySession.InvokeResult.ok(null)
      }
      MrHammadClawCanvasCommand.Eval.rawValue -> {
        val js =
      ...
      MrHammadClawCanvasA2UICommand.Push.rawValue, MrHammadClawCanvasA2UICommand.PushJSONL.rawValue -> {
        val messages =
          try {
      ...
      // Camera commands
      MrHammadClawCameraCommand.Snap.rawValue -> cameraHandler.handleSnap(paramsJson)
      MrHammadClawCameraCommand.Clip.rawValue -> cameraHandler.handleClip(paramsJson)

      // Location command
      MrHammadClawLocationCommand.Get.rawValue -> locationHandler.handleLocationGet(paramsJson)

      // Notifications command
      MrHammadClawNotificationsCommand.List.rawValue -> notificationsHandler.handleNotificationsList(paramsJson)

      // Screen command
      MrHammadClawScreenCommand.Record.rawValue -> screenHandler.handleScreenRecord(paramsJson)

      // SMS command
      MrHammadClawSmsCommand.Send.rawValue -> smsHandler.handleSmsSend(paramsJson)
