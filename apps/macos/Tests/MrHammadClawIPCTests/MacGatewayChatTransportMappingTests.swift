...
@Suite struct MacGatewayChatTransportMappingTests {
    @Test func snapshotMapsToHealth() {
        let snapshot = Snapshot(
            presence: [],
            health: MrHammadClawProtocol.AnyCodable(["ok": MrHammadClawProtocol.AnyCodable(false)]),
    ...
    @Test func healthEventMapsToHealth() {
        let frame = EventFrame(
            type: "event",
            event: "health",
            payload: MrHammadClawProtocol.AnyCodable(["ok": MrHammadClawProtocol.AnyCodable(true)]),
            seq: 1,
            stateversion: nil)
    ...
    @Test func chatEventMapsToChat() {
        let payload = MrHammadClawProtocol.AnyCodable([
            "runId": MrHammadClawProtocol.AnyCodable("run-1"),
            "sessionKey": MrHammadClawProtocol.AnyCodable("main"),
            "state": MrHammadClawProtocol.AnyCodable("final"),
        ])
    ...
    @Test func unknownEventMapsToNil() {
        let frame = EventFrame(
            type: "event",
            event: "unknown",
            payload: MrHammadClawProtocol.AnyCodable(["a": MrHammadClawProtocol.AnyCodable(1)]),
            seq: 1,
            stateversion: nil)
        let mapped = MacGatewayChatTransport.mapPushToTransportEvent(.event(frame))
        #expect(mapped == nil)
