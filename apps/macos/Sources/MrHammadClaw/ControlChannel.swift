...
struct ControlAgentEvent: Codable, Sendable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: MrHammadClawProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.mrhammadclaw", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: MrHammadClawKit.AnyCodable]()) {
                $0[$1.key] = MrHammadClawKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: MrHammadClawProtocol.AnyCodable?) -> [String: MrHammadClawProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: MrHammadClawProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: MrHammadClawKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: MrHammadClawProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("mrhammadclaw.control.heartbeat")
    static let controlAgentEvent = Notification.Name("mrhammadclaw.control.agent")
}
