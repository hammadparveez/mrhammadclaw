...
private let deepLinkLogger = Logger(subsystem: "ai.mrhammadclaw", category: "DeepLink")
...
final class DeepLinkHandler {
    ...
    func handle(url: URL) async {
        ...
        guard !AppStateStore.shared.isPaused else {
            self.presentAlert(title: "MrHammadClaw is paused", message: "Unpause MrHammadClaw to run agent actions.")
            return
        }
    ...
    private func handleAgent(link: AgentDeepLink, originalURL: URL) async {
        ...
        if !allowUnattended {
            ...
            guard self.confirm(title: "Run MrHammadClaw agent?", message: body) else { return }
        }
