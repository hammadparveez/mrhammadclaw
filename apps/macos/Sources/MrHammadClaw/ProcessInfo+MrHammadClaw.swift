...
extension ProcessInfo {
    ...
    /// Nix deployments may write defaults into a stable suite (`ai.mrhammadclaw.mac`) even if the shipped
    ...
    {
        if environment["MRHAMMADCLAW_NIX_MODE"] == "1" { return true }
        if standard.bool(forKey: "mrhammadclaw.nixMode") { return true }
        ...
        if isAppBundle, let stableSuite, stableSuite.bool(forKey: "mrhammadclaw.nixMode") { return true }
