...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["MRHAMMADCLAW_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        return URL(fileURLWithPath: "/tmp/mrhammadclaw")
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("mrhammadclaw-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("mrhammadclaw-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/mrhammadclaw/ (rolling or stdout), or nil if none exist.
