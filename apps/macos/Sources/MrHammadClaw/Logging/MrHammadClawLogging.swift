...
enum MrHammadClawLogging {
    private static let labelSeparator = "::"

    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            let (subsystem, category) = Self.parseLabel(label)
            let osHandler = MrHammadClawOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = MrHammadClawFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        MrHammadClawLogging.bootstrapIfNeeded()
        let label = MrHammadClawLogging.makeLabel(subsystem: subsystem, category: category)
...
struct MrHammadClawOSLogHandler: LogHandler {
...
struct MrHammadClawFileLogHandler: LogHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = MrHammadClawLogging.parseLabel(self.label)
