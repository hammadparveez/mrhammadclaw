...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.mrhammadclaw", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> MrHammadClawSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: MrHammadClawSessionPreviewEntry,
    ...
    private static func cache(payload: MrHammadClawSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: MrHammadClawChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [MrHammadClawKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [MrHammadClawKit.AnyCodable]) -> [MrHammadClawChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(MrHammadClawChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: MrHammadClawChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: MrHammadClawChatMessage) -> Bool {
    ...
    private static func toolNames(for message: MrHammadClawChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: MrHammadClawChatMessage) -> String? {
