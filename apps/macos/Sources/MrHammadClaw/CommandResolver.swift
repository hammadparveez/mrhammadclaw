...
enum CommandResolver {
    private static let projectRootDefaultsKey = "mrhammadclaw.gatewayProjectRootPath"
    private static let helperName = "mrhammadclaw"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let mrhammadclawEntry = root.appendingPathComponent("mrhammadclaw.mjs").path
        if FileManager().isReadableFile(atPath: mrhammadclawEntry) { return mrhammadclawEntry }
        let binEntry = root.appendingPathComponent("bin/mrhammadclaw.js").path
    ...
    static func errorCommand(with message: String) -> [String] {
        let script = """
        cat <<'__MRHAMMADCLAW_ERR__' >&2
        \(message)
        __MRHAMMADCLAW_ERR__
        exit 1
        """
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/mrhammadclaw")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let mrhammadclawPaths = self.mrhammadclawManagedPaths(home: home)
        if !mrhammadclawPaths.isEmpty {
            extras.insert(contentsOf: mrhammadclawPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + mrhammadclawPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func mrhammadclawManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".mrhammadclaw"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func mrhammadclawExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectMrHammadClawExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("mrhammadclaw.mjs").path,
            root.appendingPathComponent("bin/mrhammadclaw.js").path,
        ]
    ...
    static func hasAnyMrHammadClawInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.mrhammadclawExecutable(searchPaths: searchPaths) != nil { return true }
    ...
    static func mrhammadclawNodeCommand(
        subcommand: String,
        extraArgs: [String] = [],
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        let root = self.projectRoot()
        if let mrhammadclawPath = self.projectMrHammadClawExecutable(projectRoot: root) {
            return [mrhammadclawPath, subcommand] + extraArgs
        }
        if let mrhammadclawPath = self.mrhammadclawExecutable(searchPaths: searchPaths) {
            return [mrhammadclawPath, subcommand] + extraArgs
        }
        ...
        if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
            ...
            return [pnpm, "--silent", "mrhammadclaw", subcommand] + extraArgs
        }

        switch runtimeResult {
        case .success:
            let missingEntry = """
            mrhammadclaw entrypoint missing (looked for dist/index.js or mrhammadclaw.mjs); run pnpm build.
            """
    ...
    static func mrhammadclawCommand(
        subcommand: String,
        extraArgs: [String] = [],
        defaults: UserDefaults = .standard,
        configRoot: [String: Any]? = nil,
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real mrhammadclaw CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/mrhammadclaw"
        ...
        \(cliSection)
        \(projectSection)
        if command -v mrhammadclaw >/dev/null 2>&1; then
          CLI="$(command -v mrhammadclaw)"
          mrhammadclaw \(quotedArgs);
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/mrhammadclaw.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/mrhammadclaw.mjs"
            node "$PRJ/mrhammadclaw.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/mrhammadclaw.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/mrhammadclaw.js"
            node "$PRJ/bin/mrhammadclaw.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent mrhammadclaw"
          pnpm --silent mrhammadclaw \(quotedArgs);
        else
          echo "mrhammadclaw CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? MrHammadClawConfigFile.loadDict()
