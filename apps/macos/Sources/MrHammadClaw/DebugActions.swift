...
enum DebugActions {
    private static let verboseDefaultsKey = "mrhammadclaw.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "mrhammadclaw.onboardingSeen"
    ...
    static func openConfigFolder() {
        let url = MrHammadClawPaths.stateDirURL
        NSWorkspace.shared.activateFileViewerSelecting([url])
    }
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "MrHammadClaw", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = MrHammadClawPaths.configURL
        guard
