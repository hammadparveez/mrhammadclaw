...
private let chatUILogger = Logger(subsystem: "ai.mrhammadclaw", category: "MrHammadClawChatUI")

@MainActor
@Observable
public final class MrHammadClawChatViewModel {
    public private(set) var messages: [MrHammadClawChatMessage] = []
    ...
    public var attachments: [MrHammadClawPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [MrHammadClawChatPendingToolCall] = []
    public private(set) var sessions: [MrHammadClawChatSessionEntry] = []
    private let transport: any MrHammadClawChatTransport
    ...
    private var pendingToolCallsById: [String: MrHammadClawChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(sessionKey: String, transport: any MrHammadClawChatTransport) {
    ...
    public var sessionChoices: [MrHammadClawChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }

        var result: [MrHammadClawChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: MrHammadClawPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [MrHammadClawChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: MrHammadClawChatMessage.self))
                .map { Self.stripInboundMetadata(from: $0) }
        }
        return Self.dedupeMessages(decoded)
    }

    private static func stripInboundMetadata(from message: MrHammadClawChatMessage) -> MrHammadClawChatMessage {
        guard message.role.lowercased() == "user" else {
            return message
        }

        let sanitizedContent = message.content.map { content -> MrHammadClawChatMessageContent in
            guard let text = content.text else { return content }
            let cleaned = ChatMarkdownPreprocessor.preprocess(markdown: text).cleaned
            return MrHammadClawChatMessageContent(
                type: content.type,
                text: cleaned,
        ...
        return MrHammadClawChatMessage(
            id: message.id,
            role: message.role,
    ...
    private static func messageIdentityKey(for message: MrHammadClawChatMessage) -> String? {
    ...
    private static func reconcileMessageIDs(
        previous: [MrHammadClawChatMessage],
        incoming: [MrHammadClawChatMessage]) -> [MrHammadClawChatMessage]
    {
    ...
    private static func dedupeMessages(_ messages: [MrHammadClawChatMessage]) -> [MrHammadClawChatMessage] {
        var result: [MrHammadClawChatMessage] = []
    ...
    private static func dedupeKey(for message: MrHammadClawChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [MrHammadClawChatMessageContent] = [
            MrHammadClawChatMessageContent(
                type: "text",
                text: messageText,
                thinking: nil,
        ...
        let encodedAttachments = self.attachments.map { att -> MrHammadClawChatAttachmentPayload in
            MrHammadClawChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> MrHammadClawChatSessionEntry {
        MrHammadClawChatSessionEntry(
            key: key,
            kind: nil,
            displayName: nil,
            surface: nil,
    ...
    private func handleTransportEvent(_ evt: MrHammadClawChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: MrHammadClawChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: MrHammadClawAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = MrHammadClawChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> MrHammadClawPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
