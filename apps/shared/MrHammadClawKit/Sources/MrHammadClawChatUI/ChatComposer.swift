...
@MainActor
struct MrHammadClawChatComposer: View {
    @Bindable var viewModel: MrHammadClawChatViewModel
        ...
        .background {
            ...
            if self.style == .standard {
                let shape = UnevenRoundedRectangle(
                    ...
                    style: .continuous)
                shape
                    .fill(MrHammadClawChatTheme.composerBackground)
                    .overlay(shape.strokeBorder(MrHammadClawChatTheme.composerBorder, lineWidth: 1))
        ...
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 6) {
                ForEach(
                    ...
                    id: \MrHammadClawPendingAttachment.id)
                { (att: MrHammadClawPendingAttachment) in
                    HStack(spacing: 6) {
                        if let img = att.preview {
                            MrHammadClawPlatformImageFactory.image(img)
                                .resizable()
                                .scaledToFill()
    ...
    private var editor: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            if !self.isComposerCompacted {
                Rectangle()
                    .fill(MrHammadClawChatTheme.divider)
                    .frame(height: 1)
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(MrHammadClawChatTheme.composerField)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
                        .strokeBorder(MrHammadClawChatTheme.composerBorder)))
    ...
    private var connectionPill: some View {
        ...
        .padding(.horizontal, 8)
        .padding(.vertical, 4)
        .background(MrHammadClawChatTheme.subtleCard)
        .clipShape(Capsule())
    }
