...
import Foundation
import Testing
@testable import MrHammadClawChatUI
...
private final class TestChatTransport: @unchecked Sendable, MrHammadClawChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [MrHammadClawChatHistoryPayload]
    private let sessionsResponses: [MrHammadClawChatSessionsListResponse]

    private let stream: AsyncStream<MrHammadClawChatTransportEvent>
    private let continuation: AsyncStream<MrHammadClawChatTransportEvent>.Continuation

    init(
        historyResponses: [MrHammadClawChatHistoryPayload],
        sessionsResponses: [MrHammadClawChatSessionsListResponse] = [])
    {
        self.historyResponses = historyResponses
        self.sessionsResponses = sessionsResponses
        var cont: AsyncStream<MrHammadClawChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> MrHammadClawChatHistoryPayload {
        ...
        return self.historyResponses.last ?? MrHammadClawChatHistoryPayload(
    ...
    func sendMessage(
        ...
        attachments _: [MrHammadClawChatAttachmentPayload]) async throws -> MrHammadClawChatSendResponse
    {
        await self.state.sentRunIdsAppend(idempotencyKey)
        return MrHammadClawChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> MrHammadClawChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? MrHammadClawChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: MrHammadClawChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    @Test func streamsAssistantAndClearsOnFinal() async throws {
        ...
        let history1 = MrHammadClawChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
        let history2 = MrHammadClawChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [
                AnyCodable([
        ...
        let vm = await MainActor.run { MrHammadClawChatViewModel(sessionKey: "main", transport: transport) }
    ...
    @Test func clearsStreamingOnExternalFinalEvent() async throws {
        let sessionId = "sess-main"
        let history = MrHammadClawChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let history = MrHammadClawChatHistoryPayload(
            ...
            messages: [],
            thinkingLevel: "off")
        let sessions = MrHammadClawChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesIncludeCurrentWhenMissing() async throws {
        ...
        let transport = TestChatTransport(
            historyResponses: [history],
            sessionsResponses: [sessions])
        let vm = await MainActor.run { MrHammadClawChatViewModel(sessionKey: "custom", transport: transport) }
        await MainActor.run { vm.load() }
        try await waitUntil("sessions loaded") { await MainActor.run { !vm.sessions.isEmpty } }
