...
final class TalkModeManager: NSObject {
    ...
    private static let redactedConfigSentinel = "__MRHAMMADCLAW_REDACTED__"
    ...
    private var pttCompletion: CheckedContinuation<MrHammadClawTalkPTTStopPayload, Never>?
    ...
    private let logger = Logger(subsystem: "ai.mrhammadclaw", category: "TalkMode")
    ...
    func stop() {
        ...
        if pendingPTT {
            let payload = MrHammadClawTalkPTTStopPayload(
    ...
    func beginPushToTalk() async throws -> MrHammadClawTalkPTTStartPayload {
        guard self.gatewayConnected else {
            self.statusText = "Offline"
        ...
        if self.isPushToTalkActive, let captureId = self.activePTTCaptureId {
            return MrHammadClawTalkPTTStartPayload(captureId: captureId)
        }
    ...
    func endPushToTalk() async -> MrHammadClawTalkPTTStopPayload {
    ...
    func runPushToTalkOnce(maxDurationSeconds: TimeInterval = 12) async throws -> MrHammadClawTalkPTTStopPayload {
        ...
        if self.isPushToTalkActive {
            let captureId = self.activePTTCaptureId ?? UUID().uuidString
            return MrHammadClawTalkPTTStopPayload(
                captureId: captureId,
                transcript: nil,
                status: "busy")
        }
    ...
    func cancelPushToTalk() async -> MrHammadClawTalkPTTStopPayload {
        let captureId = self.activePTTCaptureId ?? UUID().uuidString
        guard self.isPushToTalkActive else {
            let payload = MrHammadClawTalkPTTStopPayload(
                captureId: captureId,
    ...
    private func finishPTTOnce(_ payload: MrHammadClawTalkPTTStopPayload) {
    ...
    private func streamAssistant(runId: String, gateway: GatewayNodeSession) async {
        ...
        for await evt in stream {
            ...
            guard let agentEvent = try? GatewayPayloadDecoding.decode(payload, as: MrHammadClawAgentEventPayload.self) else {
                continue
            }
