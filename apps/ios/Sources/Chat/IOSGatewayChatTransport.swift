...
struct IOSGatewayChatTransport: MrHammadClawChatTransport, Sendable {
    private static let logger = Logger(subsystem: "ai.mrhammadclaw", category: "ios.chat.transport")
    ...
    func listSessions(limit: Int?) async throws -> MrHammadClawChatSessionsListResponse {
        ...
        return try JSONDecoder().decode(MrHammadClawChatSessionsListResponse.self, from: res)
    }
    ...
    func requestHistory(sessionKey: String) async throws -> MrHammadClawChatHistoryPayload {
        ...
        return try JSONDecoder().decode(MrHammadClawChatHistoryPayload.self, from: res)
    }
        ...
        attachments: [MrHammadClawChatAttachmentPayload]) async throws -> MrHammadClawChatSendResponse
    {
        ...
        struct Params: Codable {
            ...
            var message: String
            var thinking: String
            var attachments: [MrHammadClawChatAttachmentPayload]?
            var timeoutMs: Int
        ...
        do {
            ...
            let decoded = try JSONDecoder().decode(MrHammadClawChatSendResponse.self, from: res)
    ...
    func requestHealth(timeoutMs: Int) async throws -> Bool {
        ...
        return (try? JSONDecoder().decode(MrHammadClawGatewayHealthOK.self, from: res))?.ok ?? true
    }
                ...
                for await evt in stream {
                    ...
                    case "health":
                        ...
                        let ok = (try? GatewayPayloadDecoding.decode(
                            payload,
                            as: MrHammadClawGatewayHealthOK.self))?.ok ?? true
                        continuation.yield(.health(ok: ok))
                    case "chat":
                        guard let payload = evt.payload else { break }
                        if let chatPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: MrHammadClawChatEventPayload.self)
                        {
                    ...
                    case "agent":
                        ...
                        if let agentPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: MrHammadClawAgentEventPayload.self)
                        {
