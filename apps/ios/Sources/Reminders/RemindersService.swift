...
final class RemindersService: RemindersServicing {
    func list(params: MrHammadClawRemindersListParams) async throws -> MrHammadClawRemindersListPayload {
        ...
        let payload = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<[MrHammadClawReminderPayload], Error>) in
            store.fetchReminders(matching: predicate) { items in
                ...
                let payload = selected.map { reminder in
                    ...
                    return MrHammadClawReminderPayload(
        ...
        return MrHammadClawRemindersListPayload(reminders: payload)
    }
    ...
    func add(params: MrHammadClawRemindersAddParams) async throws -> MrHammadClawRemindersAddPayload {
        ...
        let formatter = ISO8601DateFormatter()
        let due = reminder.dueDateComponents.flatMap { Calendar.current.date(from: $0) }
        let payload = MrHammadClawReminderPayload(
            identifier: reminder.calendarItemIdentifier,
            title: reminder.title,
            dueISO: due.map { formatter.string(from: $0) },
            completed: reminder.isCompleted,
        ...
        return MrHammadClawRemindersAddPayload(reminder: payload)
    }
