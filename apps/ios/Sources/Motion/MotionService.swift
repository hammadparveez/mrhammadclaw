...
final class MotionService: MotionServicing {
    func activities(params: MrHammadClawMotionActivityParams) async throws -> MrHammadClawMotionActivityPayload {
        ...
        let mapped = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<[MrHammadClawMotionActivityEntry], Error>) in
            manager.queryActivityStarting(from: start, to: end, to: OperationQueue()) { activity, error in
                if let error {
                    ...
                    let entries = sliced.map { entry in
                        MrHammadClawMotionActivityEntry(
        ...
        return MrHammadClawMotionActivityPayload(activities: mapped)
    }
    ...
    func pedometer(params: MrHammadClawPedometerParams) async throws -> MrHammadClawPedometerPayload {
        ...
        let payload = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<MrHammadClawPedometerPayload, Error>) in
            pedometer.queryPedometerData(from: start, to: end) { data, error in
                if let error {
                    cont.resume(throwing: error)
                } else {
                    let formatter = ISO8601DateFormatter()
                    let payload = MrHammadClawPedometerPayload(
