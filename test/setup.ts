...
process.env.MRHAMMADCLAW_PLUGIN_MANIFEST_CACHE_MS ??= "60000";
...
import type { MrHammadClawConfig } from "../src/config/config.js";
import type { OutboundSendDeps } from "../src/infra/outbound/deliver.js";
import { withIsolatedTestHome } from "./test-env.js";

// Set HOME/state isolation before importing any runtime MrHammadClaw modules.
...
const createStubPlugin = (params: {
  ...
  config: {
    listAccountIds: (cfg: MrHammadClawConfig) => {
    ...
    resolveAccount: (cfg: MrHammadClawConfig, accountId?: string | null) => {
      const channels = cfg.channels as Record<string, unknown> | undefined;
      const entry = channels?.[params.id];
      if (!entry || typeof entry !== "object") {
        return {};
    ...
    isConfigured: async (_account, cfg: MrHammadClawConfig) => {
      const channels = cfg.channels as Record<string, unknown> | undefined;
