...
function setTempHome(base: string) {
  ...
  // Ensure tests using HOME isolation aren't affected by leaked MRHAMMADCLAW_HOME.
  delete process.env.MRHAMMADCLAW_HOME;
  process.env.MRHAMMADCLAW_STATE_DIR = path.join(base, ".mrhammadclaw");
...
export async function withTempHome<T>(
  ...
  const base = await fs.mkdtemp(path.join(os.tmpdir(), opts.prefix ?? "mrhammadclaw-test-home-"));
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".mrhammadclaw", "agents", "main", "sessions"), {
    recursive: true,
  });
  if (opts.env) {
