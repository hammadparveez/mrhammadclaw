...
export async function spawnGatewayInstance(name: string): Promise<GatewayInstance> {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `mrhammadclaw-e2e-${name}-`));
  const configDir = path.join(homeDir, ".mrhammadclaw");
    ...
    child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...
          MRHAMMADCLAW_SKIP_PROVIDERS: "1",
          MRHAMMADCLAW_SKIP_GMAIL_WATCHER: "1",
          MRHAMMADCLAW_SKIP_CRON: "1",
          MRHAMMADCLAW_SKIP_BROWSER_CONTROL_SERVER: "1",
          MRHAMMADCLAW_SKIP_CANVAS_HOST: "1",
          MRHAMMADCLAW_TEST_MINIMAL_GATEWAY: "1",
          VITEST: "1",
        },
