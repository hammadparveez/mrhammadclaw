...
import type { MrHammadClawPluginApi, MrHammadClawPluginService } from "mrhammadclaw/plugin-sdk";
...
function normalizeDenyList(cfg: MrHammadClawPluginApi["config"]): string[] {
  return uniqSorted([...(cfg.gateway?.nodes?.denyCommands ?? [])]);
}

function normalizeAllowList(cfg: MrHammadClawPluginApi["config"]): string[] {
...
export default function register(api: MrHammadClawPluginApi) {
  ...
  const timerService: MrHammadClawPluginService = {
