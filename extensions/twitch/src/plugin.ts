/**
 * Twitch channel plugin for MrHammadClaw.
 *
...
import { buildChannelConfigSchema } from "mrhammadclaw/plugin-sdk";
 ...
 * for MrHammadClaw. Supports message sending, receiving, access control, and
 * status monitoring.
 */
export const twitchPlugin: ChannelPlugin<TwitchAccountConfig> = {
  ...
  config: {
    ...
    listAccountIds: (cfg: MrHammadClawConfig): string[] => listAccountIds(cfg),

    /** Resolve an account config by ID */
    resolveAccount: (cfg: MrHammadClawConfig, accountId?: string | null): TwitchAccountConfig => {
    ...
    isConfigured: (_account: unknown, cfg: MrHammadClawConfig): boolean => {
