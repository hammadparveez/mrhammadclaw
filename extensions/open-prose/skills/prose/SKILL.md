...
metadata: { "mrhammadclaw": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == MrHammadClaw `sessions_spawn`
- **File I/O** == MrHammadClaw `read`/`write`
- **Remote fetch** == MrHammadClaw `web_fetch` (or `exec` with curl when POST is required)
