...
export function extractMSTeamsPollVote(
  ...
  const pollId =
    readNestedString(value, ["mrhammadclawPollId"]) ??
    readNestedString(value, ["pollId"]) ??
    readNestedString(value, ["mrhammadclaw", "pollId"]) ??
    readNestedString(value, ["mrhammadclaw", "poll", "id"]) ??
    readNestedString(value, ["data", "mrhammadclawPollId"]) ??
    readNestedString(value, ["data", "pollId"]) ??
    readNestedString(value, ["data", "mrhammadclaw", "pollId"]);
  if (!pollId) {
    return null;
  }
    ...
    actions: [
      {
        ...
        title: "Vote",
        data: {
          mrhammadclawPollId: pollId,
          pollId,
        },
        msteams: {
          type: "messageBack",
          text: "mrhammadclaw poll vote",
          displayText: "Vote recorded",
          value: { mrhammadclawPollId: pollId, pollId },
        },
      },
    ],
  };
