...
function resolveMrHammadClawRoot(): string {
  if (coreRootCache) {
    return coreRootCache;
  }
  const override = process.env.MRHAMMADCLAW_ROOT?.trim();
  if (override) {
  ...
  for (const start of candidates) {
    for (const name of ["mrhammadclaw"]) {
  ...
  throw new Error("Unable to resolve core root. Set MRHAMMADCLAW_ROOT to the package root.");
}
...
async function importCoreExtensionAPI(): Promise<{
  ...
  const distPath = path.join(resolveMrHammadClawRoot(), "dist", "extensionAPI.js");
