---
name: acp-router
description: Route plain-language requests for Pi, Claude Code, Codex, OpenCode, Gemini CLI, or ACP harness work into either MrHammadClaw ACP runtime sessions or direct acpx-driven sessions ("telephone game" flow).
...
Trigger this skill when the user asks MrHammadClaw to:
...
1. MrHammadClaw ACP runtime path (default): use `sessions_spawn` / ACP runtime tools.
...
- the task is "just relay prompts to harness" and no MrHammadClaw ACP lifecycle features are needed
...
For this repo, direct `acpx` calls must follow the same pinned policy as the `@mrhammadclaw/acpx` extension.
