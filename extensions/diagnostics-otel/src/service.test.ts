...
vi.mock("mrhammadclaw/plugin-sdk", async () => {
  const actual =
    await vi.importActual<typeof import("mrhammadclaw/plugin-sdk")>("mrhammadclaw/plugin-sdk");
  return {
    ...actual,
...
import type { MrHammadClawPluginServiceContext } from "mrhammadclaw/plugin-sdk";
import { emitDiagnosticEvent } from "mrhammadclaw/plugin-sdk";
import { createDiagnosticsOtelService } from "./service.js";

const OTEL_TEST_STATE_DIR = "/tmp/mrhammadclaw-diagnostics-otel-test";
...
): MrHammadClawPluginServiceContext {
  return {
    config: {
      diagnostics: {
        enabled: true,
...
function createTraceOnlyContext(endpoint: string): MrHammadClawPluginServiceContext {
...
describe("diagnostics-otel service", () => {
  ...
  test("records message-flow metrics and spans", async () => {
    ...
    expect(telemetryState.counters.get("mrhammadclaw.webhook.received")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("mrhammadclaw.webhook.duration_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.counters.get("mrhammadclaw.message.queued")?.add).toHaveBeenCalled();
    expect(telemetryState.counters.get("mrhammadclaw.message.processed")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("mrhammadclaw.message.duration_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.histograms.get("mrhammadclaw.queue.wait_ms")?.record).toHaveBeenCalled();
    expect(telemetryState.counters.get("mrhammadclaw.session.stuck")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("mrhammadclaw.session.stuck_age_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.counters.get("mrhammadclaw.run.attempt")?.add).toHaveBeenCalled();

    const spanNames = telemetryState.tracer.startSpan.mock.calls.map((call) => call[0]);
    expect(spanNames).toContain("mrhammadclaw.webhook.processed");
    expect(spanNames).toContain("mrhammadclaw.message.processed");
    expect(spanNames).toContain("mrhammadclaw.session.stuck");
  ...
  test("redacts sensitive data from log attributes before export", async () => {
    ...
    const tokenAttr = emitCall?.attributes?.["mrhammadclaw.token"];
  ...
  test("redacts sensitive reason in session.state metric attributes", async () => {
    ...
    const sessionCounter = telemetryState.counters.get("mrhammadclaw.session.state");
    expect(sessionCounter?.add).toHaveBeenCalledWith(
      1,
      expect.objectContaining({
        "mrhammadclaw.reason": expect.stringContaining("…"),
      }),
    );
    const attrs = sessionCounter?.add.mock.calls[0]?.[1] as Record<string, unknown> | undefined;
    expect(typeof attrs?.["mrhammadclaw.reason"]).toBe("string");
    expect(String(attrs?.["mrhammadclaw.reason"])).not.toContain(
