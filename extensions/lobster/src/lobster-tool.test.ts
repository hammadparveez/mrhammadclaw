...
function fakeApi(overrides: Partial<MrHammadClawPluginApi> = {}): MrHammadClawPluginApi {
  return {
    id: "lobster",
    name: "lobster",
    source: "test",
...
function fakeCtx(
  overrides: Partial<MrHammadClawPluginToolContext> = {},
): MrHammadClawPluginToolContext {
  return {
    config: {},
    workspaceDir: "/tmp",
...
describe("lobster plugin tool", () => {
  ...
  beforeAll(async () => {
    ...
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "mrhammadclaw-lobster-plugin-"));
  });
  ...
  it("can be gated off in sandboxed contexts", async () => {
    const api = fakeApi();
    const factoryTool = (ctx: MrHammadClawPluginToolContext) => {
      if (ctx.sandboxed) {
        return null;
      }
