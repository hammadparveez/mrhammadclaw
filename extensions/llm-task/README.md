...
`mrhammadclaw.invoke --each`) without adding new MrHammadClaw code per workflow.
...
This extension depends on MrHammadClaw internal modules (the embedded agent runner).
It is intended to ship as a **bundled** MrHammadClaw extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.mrhammadclaw/extensions` as a standalone plugin directory.
