...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.MRHAMMADCLAW_GATEWAY_PORT).toBe("18789");
    expect(env.MRHAMMADCLAW_GATEWAY_TOKEN).toBe("secret");
    expect(env.MRHAMMADCLAW_SERVICE_MARKER).toBe("mrhammadclaw");
    expect(env.MRHAMMADCLAW_SERVICE_KIND).toBe("gateway");
    expect(typeof env.MRHAMMADCLAW_SERVICE_VERSION).toBe("string");
    expect(env.MRHAMMADCLAW_SYSTEMD_UNIT).toBe("mrhammadclaw-gateway.service");
    if (process.platform === "darwin") {
      expect(env.MRHAMMADCLAW_LAUNCHD_LABEL).toBe("ai.mrhammadclaw.gateway");
    }
  });
  ...
  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", MRHAMMADCLAW_PROFILE: "work" },
      port: 18789,
    });
    expect(env.MRHAMMADCLAW_SYSTEMD_UNIT).toBe("mrhammadclaw-gateway-work.service");
    if (process.platform === "darwin") {
      expect(env.MRHAMMADCLAW_LAUNCHD_LABEL).toBe("ai.mrhammadclaw.work");
    }
  });
...
describe("resolveGatewayStateDir", () => {
  it("uses the default state dir when no overrides are set", () => {
    ...
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".mrhammadclaw"));
  });

  it("appends the profile suffix when set", () => {
    const env = { HOME: "/Users/test", MRHAMMADCLAW_PROFILE: "rescue" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".mrhammadclaw-rescue"));
  });

  it("treats default profiles as the base state dir", () => {
    const env = { HOME: "/Users/test", MRHAMMADCLAW_PROFILE: "Default" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".mrhammadclaw"));
  });

  it("uses MRHAMMADCLAW_STATE_DIR when provided", () => {
    const env = { HOME: "/Users/test", MRHAMMADCLAW_STATE_DIR: "/var/lib/mrhammadclaw" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/var/lib/mrhammadclaw"));
  });

  it("expands ~ in MRHAMMADCLAW_STATE_DIR", () => {
    const env = { HOME: "/Users/test", MRHAMMADCLAW_STATE_DIR: "~/mrhammadclaw-state" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/Users/test/mrhammadclaw-state"));
  });

  it("preserves Windows absolute paths without HOME", () => {
    const env = { MRHAMMADCLAW_STATE_DIR: "C:\\State\\mrhammadclaw" };
    expect(resolveGatewayStateDir(env)).toBe("C:\\State\\mrhammadclaw");
  });
});
