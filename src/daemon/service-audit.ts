...
function auditGatewayToken(
  ...
  const serviceToken = command?.environment?.MRHAMMADCLAW_GATEWAY_TOKEN?.trim();
  ...
  issues.push({
    ...
    message:
      "Gateway service MRHAMMADCLAW_GATEWAY_TOKEN does not match gateway.auth.token in mrhammadclaw.json",
...
export function checkTokenDrift(params: {
  ...
  if (configToken && serviceToken !== configToken) {
    return {
      ...
      detail: "Run `mrhammadclaw gateway install --force` to sync the token.",
      level: "recommended",
    };
  }
