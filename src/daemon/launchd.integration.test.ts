...
describeLaunchdIntegration("launchd integration", () => {
  ...
  beforeAll(async () => {
    ...
    homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `mrhammadclaw-launchd-int-${testId}-`));
    env = {
      HOME: homeDir,
      MRHAMMADCLAW_LAUNCHD_LABEL: `ai.mrhammadclaw.launchd-int-${testId}`,
      MRHAMMADCLAW_LOG_PREFIX: `gateway-launchd-int-${testId}`,
    };
