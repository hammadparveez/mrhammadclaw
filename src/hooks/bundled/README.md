...
This directory contains hooks that ship with MrHammadClaw. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
Hooks can be configured in `~/.mrhammadclaw/mrhammadclaw.json`:
...
2. Restart gateway: `pkill -9 -f 'mrhammadclaw.*gateway' && pnpm mrhammadclaw gateway`
3. Enable the hook: `mrhammadclaw hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.mrhammadclaw.ai/automation/hooks
