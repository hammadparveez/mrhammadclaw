...
homepage: https://docs.mrhammadclaw.ai/automation/hooks#command-logger
metadata:
  {
    "mrhammadclaw":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.mrhammadclaw/logs/commands.log`
...
`~/.mrhammadclaw/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.mrhammadclaw/logs/commands.log ~/.mrhammadclaw/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/mrhammadclaw`:
   ```
   /home/username/.mrhammadclaw/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
