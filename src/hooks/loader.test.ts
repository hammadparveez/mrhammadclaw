...
describe("loader", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "mrhammadclaw-hooks-loader-"));
  });
  ...
  beforeEach(async () => {
    ...
    envSnapshot = captureEnv(["MRHAMMADCLAW_BUNDLED_HOOKS_DIR"]);
    process.env.MRHAMMADCLAW_BUNDLED_HOOKS_DIR = "/nonexistent/bundled/hooks";
  });
