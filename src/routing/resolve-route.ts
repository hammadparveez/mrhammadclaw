...
function listAgents(cfg: MrHammadClawConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

function pickFirstExistingAgentId(cfg: MrHammadClawConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: MrHammadClawConfig["bindings"];
  byChannelAccount: Map<string, EvaluatedBinding[]>;
};

const evaluatedBindingsCacheByCfg = new WeakMap<MrHammadClawConfig, EvaluatedBindingsCache>();
