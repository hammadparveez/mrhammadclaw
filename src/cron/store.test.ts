...
async function makeStorePath() {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "mrhammadclaw-cron-store-"));
  return {
    dir,
...
describe("resolveCronStorePath", () => {
  ...
  it("uses MRHAMMADCLAW_HOME for tilde expansion", () => {
    vi.stubEnv("MRHAMMADCLAW_HOME", "/srv/mrhammadclaw-home");
    vi.stubEnv("HOME", "/home/other");

    const result = resolveCronStorePath("~/cron/jobs.json");
    expect(result).toBe(path.resolve("/srv/mrhammadclaw-home", "cron", "jobs.json"));
  });
});
