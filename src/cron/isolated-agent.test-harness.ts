...
export async function withTempCronHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "mrhammadclaw-cron-" });
}
...
export async function writeSessionStoreEntries(
  ...
  const dir = path.join(home, ".mrhammadclaw", "sessions");
...
export function makeCfg(
  home: string,
  storePath: string,
  overrides: Partial<MrHammadClawConfig> = {},
): MrHammadClawConfig {
  const base: MrHammadClawConfig = {
    agents: {
      defaults: {
