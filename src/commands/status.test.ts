...
beforeAll(() => {
  envSnapshot = captureEnv(["MRHAMMADCLAW_PROFILE"]);
  process.env.MRHAMMADCLAW_PROFILE = "isolated";
});
...
vi.mock("../infra/mrhammadclaw-root.js", () => ({
  resolveMrHammadClawPackageRoot: vi.fn().mockResolvedValue("/tmp/mrhammadclaw"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/mrhammadclaw/pnpm-lock.yaml",
      markerPath: "/tmp/mrhammadclaw/node_modules/.modules.yaml",
    },
...
vi.mock("../daemon/service.js", () => ({
  resolveGatewayService: () => ({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.mrhammadclaw.gateway.plist",
    }),
  }),
}));
vi.mock("../daemon/node-service.js", () => ({
  resolveNodeService: () => ({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.mrhammadclaw.node.plist",
    }),
  }),
}));
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    await statusCommand({}, runtime as never);
    const logs = runtimeLogMock.mock.calls.map((c: unknown[]) => String(c[0]));
    expect(logs.some((l: string) => l.includes("MrHammadClaw status"))).toBe(true);
    ...
    expect(
      logs.some(
        (l: string) =>
          l.includes("mrhammadclaw status --all") ||
          l.includes("mrhammadclaw --profile isolated status --all") ||
          l.includes("mrhammadclaw status --all") ||
          l.includes("mrhammadclaw --profile isolated status --all"),
      ),
    ).toBe(true);
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    process.env.MRHAMMADCLAW_GATEWAY_TOKEN = "abcd1234";
    try {
