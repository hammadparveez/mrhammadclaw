...
export async function statusCommand(
  ...
  runtime.log(theme.heading("MrHammadClaw status"));
  runtime.log("");
  ...
  if (pairingRecovery) {
    ...
    if (pairingRecovery.requestId) {
      runtime.log(
        theme.muted(
          `Recovery: ${formatCliCommand(`mrhammadclaw devices approve ${pairingRecovery.requestId}`)}`,
        ),
      );
    }
    runtime.log(
      theme.muted(`Fallback: ${formatCliCommand("mrhammadclaw devices approve --latest")}`),
    );
    runtime.log(theme.muted(`Inspect: ${formatCliCommand("mrhammadclaw devices list")}`));
  }
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("mrhammadclaw security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("mrhammadclaw security audit --deep")}`));
  ...
  runtime.log("");
  runtime.log("FAQ: https://docs.mrhammadclaw.ai/faq");
  runtime.log("Troubleshooting: https://docs.mrhammadclaw.ai/troubleshooting");
  runtime.log("");
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("mrhammadclaw status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("mrhammadclaw logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("mrhammadclaw status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("mrhammadclaw gateway probe")}`);
  }
}
