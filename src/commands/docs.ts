...
const SEARCH_TOOL = "https://docs.mrhammadclaw.ai/mcp.SearchMrHammadClaw";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.mrhammadclaw.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(
        `${theme.muted("Search:")} ${formatCliCommand('mrhammadclaw docs "your query"')}`,
      );
    } else {
      runtime.log("Docs: https://docs.mrhammadclaw.ai/");
      runtime.log(`Search: ${formatCliCommand('mrhammadclaw docs "your query"')}`);
    }
    return;
  }
