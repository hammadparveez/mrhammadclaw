...
describe("writeOAuthCredentials", () => {
  const lifecycle = createAuthTestLifecycle([
    ...
    "MRHAMMADCLAW_AGENT_DIR",
    "PI_CODING_AGENT_DIR",
    "MRHAMMADCLAW_OAUTH_DIR",
  ]);
  ...
  it("writes auth-profiles.json under MRHAMMADCLAW_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("mrhammadclaw-oauth-");
  ...
  it("writes OAuth credentials to all sibling agent dirs when syncSiblingAgents=true", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "mrhammadclaw-oauth-sync-"));
    ...
    process.env.MRHAMMADCLAW_AGENT_DIR = kidAgentDir;
  ...
  it("writes OAuth credentials only to target dir by default", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "mrhammadclaw-oauth-nosync-"));
    process.env.MRHAMMADCLAW_STATE_DIR = tempStateDir;
  ...
  it("syncs siblings from explicit agentDir outside MRHAMMADCLAW_STATE_DIR", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "mrhammadclaw-oauth-external-"));
    process.env.MRHAMMADCLAW_STATE_DIR = tempStateDir;

    // Create standard-layout agents tree *outside* MRHAMMADCLAW_STATE_DIR
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to MRHAMMADCLAW_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("mrhammadclaw-minimax-", { agentSubdir: "custom-agent" });
    lifecycle.setStateDir(env.stateDir);
