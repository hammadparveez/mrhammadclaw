...
type PromptDefaultModelResult = { model?: string; config?: MrHammadClawConfig };
...
function resolveConfiguredModelRaw(cfg: MrHammadClawConfig): string {
  return resolveAgentModelPrimaryValue(cfg.agents?.defaults?.model) ?? "";
}

function resolveConfiguredModelKeys(cfg: MrHammadClawConfig): string[] {
...
export function applyPrimaryModel(cfg: MrHammadClawConfig, model: string): MrHammadClawConfig {
...
export function applyModelAllowlist(cfg: MrHammadClawConfig, models: string[]): MrHammadClawConfig {
  const defaults = cfg.agents?.defaults;
