...
export function requireMrHammadClawAgentDir(): string {
  const agentDir = process.env.MRHAMMADCLAW_AGENT_DIR;
  if (!agentDir) {
    throw new Error("MRHAMMADCLAW_AGENT_DIR not set");
  }
  return agentDir;
}
