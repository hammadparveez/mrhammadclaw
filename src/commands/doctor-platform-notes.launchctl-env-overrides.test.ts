...
describe("noteMacLaunchctlGatewayEnvOverrides", () => {
  it("prints clear unsetenv instructions for token override", async () => {
    ...
    const getenv = vi.fn(async (name: string) =>
      name === "MRHAMMADCLAW_GATEWAY_TOKEN" ? "launchctl-token" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(message).toContain("MRHAMMADCLAW_GATEWAY_TOKEN");
    expect(message).toContain("launchctl unsetenv MRHAMMADCLAW_GATEWAY_TOKEN");
    expect(message).not.toContain("MRHAMMADCLAW_GATEWAY_PASSWORD");
  });
