...
function resolveAgentName(cfg: MrHammadClawConfig, agentId: string) {
...
function resolveAgentModel(cfg: MrHammadClawConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: MrHammadClawConfig): AgentSummary[] {
