...
function resolveMode(cfg: MrHammadClawConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  intro("MrHammadClaw doctor");

  const root = await resolveMrHammadClawPackageRoot({
  ...
  let cfg: MrHammadClawConfig = configResult.cfg;
  ...
  if (!cfg.gateway?.mode) {
    const lines = [
      ...
      `Fix: run ${formatCliCommand("mrhammadclaw configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("mrhammadclaw config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("mrhammadclaw setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("mrhammadclaw doctor --fix")}" to apply changes.`);
  }
