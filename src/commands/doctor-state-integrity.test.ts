...
type EnvSnapshot = {
  HOME?: string;
  MRHAMMADCLAW_HOME?: string;
  MRHAMMADCLAW_STATE_DIR?: string;
  MRHAMMADCLAW_OAUTH_DIR?: string;
};

function captureEnv(): EnvSnapshot {
  return {
    HOME: process.env.HOME,
    MRHAMMADCLAW_HOME: process.env.MRHAMMADCLAW_HOME,
    MRHAMMADCLAW_STATE_DIR: process.env.MRHAMMADCLAW_STATE_DIR,
    MRHAMMADCLAW_OAUTH_DIR: process.env.MRHAMMADCLAW_OAUTH_DIR,
  };
}
...
function setupSessionState(cfg: MrHammadClawConfig, env: NodeJS.ProcessEnv, homeDir: string) {
  const agentId = "main";
...
async function runStateIntegrity(cfg: MrHammadClawConfig) {
...
describe("doctor state integrity oauth dir checks", () => {
  ...
  beforeEach(() => {
    ...
    tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "mrhammadclaw-doctor-state-integrity-"));
    process.env.HOME = tempHome;
    process.env.MRHAMMADCLAW_HOME = tempHome;
    process.env.MRHAMMADCLAW_STATE_DIR = path.join(tempHome, ".mrhammadclaw");
    delete process.env.MRHAMMADCLAW_OAUTH_DIR;
    fs.mkdirSync(process.env.MRHAMMADCLAW_STATE_DIR, { recursive: true, mode: 0o700 });
  ...
  it("prompts for oauth dir when MRHAMMADCLAW_OAUTH_DIR is explicitly configured", async () => {
    process.env.MRHAMMADCLAW_OAUTH_DIR = path.join(tempHome, ".oauth");
    const cfg: MrHammadClawConfig = {};
    const confirmSkipInNonInteractive = await runStateIntegrity(cfg);
    expect(confirmSkipInNonInteractive).toHaveBeenCalledWith(OAUTH_PROMPT_MATCHER);
    expect(stateIntegrityText()).toContain("CRITICAL: OAuth dir missing");
  ...
  it("prints mrhammadclaw-only verification hints when recent sessions are missing transcripts", async () => {
    const cfg: MrHammadClawConfig = {};
    setupSessionState(cfg, process.env, process.env.HOME ?? "");
    ...
    expect(text).toMatch(/mrhammadclaw sessions --store ".*sessions\.json"/);
    expect(text).toMatch(/mrhammadclaw sessions cleanup --store ".*sessions\.json" --dry-run/);
    expect(text).toMatch(
      /mrhammadclaw sessions cleanup --store ".*sessions\.json" --enforce --fix-missing/,
    );
