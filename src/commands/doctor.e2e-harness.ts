...
export const resolveMrHammadClawPackageRoot = vi.fn().mockResolvedValue(null) as unknown as MockFn;
...
vi.mock("../plugins/loader.js", () => ({
  loadMrHammadClawPlugins: () => ({ plugins: [], diagnostics: [] }),
}));
...
vi.mock("../config/config.js", async (importOriginal) => {
  ...
  return {
    ...actual,
    CONFIG_PATH: "/tmp/mrhammadclaw.json",
    createConfigIO,
...
vi.mock("../infra/mrhammadclaw-root.js", () => ({
  resolveMrHammadClawPackageRoot,
}));
...
beforeEach(() => {
  ...
  resolveMrHammadClawPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.MRHAMMADCLAW_STATE_DIR;
  originalUpdateInProgress = process.env.MRHAMMADCLAW_UPDATE_IN_PROGRESS;
  process.env.MRHAMMADCLAW_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "mrhammadclaw-doctor-state-"));
...
afterEach(() => {
  ...
  if (originalStateDir === undefined) {
    ...
    process.env.MRHAMMADCLAW_STATE_DIR = originalStateDir;
  }
  if (originalUpdateInProgress === undefined) {
    delete process.env.MRHAMMADCLAW_UPDATE_IN_PROGRESS;
  } else {
    process.env.MRHAMMADCLAW_UPDATE_IN_PROGRESS = originalUpdateInProgress;
  }
  if (tempStateDir) {
