...
function expectPrimaryModelChanged(
  applied: { changed: boolean; next: MrHammadClawConfig },
  primary: string,
) {
...
type SharedDefaultModelCase = {
  apply: (cfg: MrHammadClawConfig) => { changed: boolean; next: MrHammadClawConfig };
  defaultModel: string;
  overrideConfig: MrHammadClawConfig;
  alreadyDefaultConfig: MrHammadClawConfig;
};
...
describe("applyDefaultModelChoice", () => {
  it("ensures allowlist entry exists when returning an agent override", async () => {
    ...
    const applied = await applyDefaultModelChoice({
      ...
      applyProviderConfig: (config: MrHammadClawConfig) => config,
      applyDefaultConfig: (config: MrHammadClawConfig) => config,
      noteAgentModel,
...
describe("shared default model behavior", () => {
  it("sets defaults when model is unset", () => {
    for (const testCase of SHARED_DEFAULT_MODEL_CASES) {
      const cfg: MrHammadClawConfig = { agents: { defaults: {} } };
