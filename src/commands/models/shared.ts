...
export async function loadValidConfigOrThrow(): Promise<MrHammadClawConfig> {
...
export async function updateConfig(
  mutator: (cfg: MrHammadClawConfig) => MrHammadClawConfig,
...
export function resolveModelTarget(params: { raw: string; cfg: MrHammadClawConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: MrHammadClawConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("mrhammadclaw agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
