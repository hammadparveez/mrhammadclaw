...
export async function modelsStatusCommand(
  ...
  const agentDir = agentId ? resolveAgentDir(cfg, agentId) : resolveMrHammadClawAgentDir();
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("mrhammadclaw models auth setup-token")}\` or \`${formatCliCommand("mrhammadclaw configure")}\`.`
          : `Run \`${formatCliCommand("mrhammadclaw configure")}\` or set an API key env var.`;
