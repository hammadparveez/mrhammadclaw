...
export function restoreGatewayPortEnv(prevGatewayPort: string | undefined): void {
  ...
  process.env.MRHAMMADCLAW_GATEWAY_PORT = prevGatewayPort;
}
...
const chromeUserDataDir = vi.hoisted(() => ({ dir: "/tmp/mrhammadclaw" }));
  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        headless: true,
        defaultProfile: "mrhammadclaw",
        profiles: {
          mrhammadclaw: { cdpPort: state.testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
vi.mock("./chrome.js", () => ({
  ...
  launchMrHammadClawChrome: vi.fn(async (_resolved: unknown, profile: { cdpPort: number }) => {
...
export async function resetBrowserControlServerTestContext(): Promise<void> {
  ...
  state.prevGatewayPort = process.env.MRHAMMADCLAW_GATEWAY_PORT;
  process.env.MRHAMMADCLAW_GATEWAY_PORT = String(state.testPort - 2);
  ...
  state.prevGatewayToken = process.env.MRHAMMADCLAW_GATEWAY_TOKEN;
  state.prevGatewayPassword = process.env.MRHAMMADCLAW_GATEWAY_PASSWORD;
...
export function restoreGatewayAuthEnv(
  ...
  if (prevGatewayToken === undefined) {
    ...
    process.env.MRHAMMADCLAW_GATEWAY_TOKEN = prevGatewayToken;
  }
  if (prevGatewayPassword === undefined) {
    delete process.env.MRHAMMADCLAW_GATEWAY_PASSWORD;
  } else {
    process.env.MRHAMMADCLAW_GATEWAY_PASSWORD = prevGatewayPassword;
  }
}
