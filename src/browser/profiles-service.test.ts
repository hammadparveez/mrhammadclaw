...
vi.mock("./chrome.js", () => ({
  resolveMrHammadClawUserDataDir: vi.fn(() => "/tmp/mrhammadclaw-test/mrhammadclaw/user-data"),
}));

import { loadConfig, writeConfigFile } from "../config/config.js";
import { resolveMrHammadClawUserDataDir } from "./chrome.js";
...
describe("BrowserProfilesService", () => {
  ...
  it("deletes local profiles and moves data to Trash", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join("/tmp", "mrhammadclaw-profile-"));
    const userDataDir = path.join(tempDir, "work", "user-data");
    fs.mkdirSync(path.dirname(userDataDir), { recursive: true });
    vi.mocked(resolveMrHammadClawUserDataDir).mockReturnValue(userDataDir);
