  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        profiles: {
          mrhammadclaw: { cdpPort: testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
describe("browser control evaluate gating", () => {
  beforeEach(async () => {
    ...
    prevGatewayPort = process.env.MRHAMMADCLAW_GATEWAY_PORT;
    process.env.MRHAMMADCLAW_GATEWAY_PORT = String(testPort - 2);
    prevGatewayToken = process.env.MRHAMMADCLAW_GATEWAY_TOKEN;
    prevGatewayPassword = process.env.MRHAMMADCLAW_GATEWAY_PASSWORD;
