import type { BrowserConfig, BrowserProfileConfig, MrHammadClawConfig } from "../config/config.js";
...
import {
  ...
  DEFAULT_MRHAMMADCLAW_BROWSER_ENABLED,
...
export type ResolvedBrowserProfile = {
  ...
  cdpIsLoopback: boolean;
  color: string;
  driver: "mrhammadclaw" | "extension";
};

function normalizeHexColor(raw: string | undefined) {
  const value = (raw ?? "").trim();
  if (!value) {
    return DEFAULT_MRHAMMADCLAW_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "mrhammadclaw" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_MRHAMMADCLAW_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_MRHAMMADCLAW_BROWSER_PROFILE_NAME] = {
 ...
 *
 * Note: this is an MrHammadClaw browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for mrhammadclaw/mrhammadclaw CDP).
...
export function resolveBrowserConfig(
  ...
  rootConfig?: MrHammadClawConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_MRHAMMADCLAW_BROWSER_ENABLED;
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "mrhammadclaw";
