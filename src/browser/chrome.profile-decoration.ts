...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".mrhammadclaw-profile-decorated");
}
 ...
 */
export function decorateMrHammadClawProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_MRHAMMADCLAW_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_MRHAMMADCLAW_BROWSER_COLOR).toUpperCase();
