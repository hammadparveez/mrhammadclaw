...
const tmpDirMocks = vi.hoisted(() => ({
  resolvePreferredMrHammadClawTmpDir: vi.fn(() => "/tmp/mrhammadclaw"),
}));
vi.mock("../infra/tmp-mrhammadclaw-dir.js", () => tmpDirMocks);
...
describe("pw-tools-core", () => {
  beforeEach(() => {
    ...
    tmpDirMocks.resolvePreferredMrHammadClawTmpDir.mockReturnValue("/tmp/mrhammadclaw");
  });
  ...
  it("uses preferred tmp dir when waiting for download without explicit path", async () => {
    tmpDirMocks.resolvePreferredMrHammadClawTmpDir.mockReturnValue("/tmp/mrhammadclaw-preferred");
    ...
    const expectedRootedDownloadsDir = path.join(
      path.sep,
      "tmp",
      "mrhammadclaw-preferred",
      "downloads",
    );
    const expectedDownloadsTail = `${path.join("tmp", "mrhammadclaw-preferred", "downloads")}${path.sep}`;
    ...
    expect(tmpDirMocks.resolvePreferredMrHammadClawTmpDir).toHaveBeenCalled();
  });
  ...
  it("sanitizes suggested download filenames to prevent traversal escapes", async () => {
    ...
    expect(path.dirname(String(outPath))).toBe(
      path.join(path.sep, "tmp", "mrhammadclaw-preferred", "downloads"),
    );
    expect(path.basename(String(outPath))).toMatch(/-passwd$/);
    expect(path.normalize(res.path)).toContain(
      path.normalize(`${path.join("tmp", "mrhammadclaw-preferred", "downloads")}${path.sep}`),
    );
  });
