...
describe("agent components", () => {
  const createCfg = (): MrHammadClawConfig => ({}) as MrHammadClawConfig;
...
describe("discord component interactions", () => {
  const createCfg = (): MrHammadClawConfig =>
    ({
      channels: {
        discord: {
          replyToMode: "first",
  ...
  beforeEach(() => {
    ...
    resolveStorePathMock.mockClear().mockReturnValue("/tmp/mrhammadclaw-sessions-test.json");
  });
...
describe("resolveDiscordPresenceUpdate", () => {
  ...
  it("includes streaming url when activityType is streaming", () => {
    ...
    expect(presence?.activities[0]).toMatchObject({
      type: 1,
      name: "Live",
      url: "https://twitch.tv/mrhammadclaw",
    });
  });
});
