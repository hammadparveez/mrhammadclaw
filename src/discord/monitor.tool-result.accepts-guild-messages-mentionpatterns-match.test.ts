  ...
  return {
    agents: {
      defaults: {
        ...
        workspace: "/tmp/mrhammadclaw",
      },
    },
    session: { store: "/tmp/mrhammadclaw-sessions.json" },
...
describe("discord tool result dispatch", () => {
  it(
    ...
    async () => {
      const cfg = createMentionRequiredGuildConfig({
        messages: {
          responsePrefix: "PFX",
          groupChat: { mentionPatterns: ["\\bmrhammadclaw\\b"] },
        },
      });
      ...
      await handler(
        createGuildMessageEvent({ messageId: "m2", content: "mrhammadclaw: hello" }),
        client,
      );
      ...
      createGuildMessageEvent({
        ...
        messagePatch: {
          referencedMessage: {
            ...
            mentionedUsers: [],
            mentionedRoles: [],
            author: { id: "bot-id", bot: true, username: "MrHammadClaw" },
          },
        },
        eventPatch: {
  ...
  it("treats forum threads as distinct sessions without channel payloads", async () => {
    ...
    const cfg = {
      agent: { model: "anthropic/claude-opus-4-5", workspace: "/tmp/mrhammadclaw" },
      session: { store: "/tmp/mrhammadclaw-sessions.json" },
      channels: {
        discord: {
          dm: { enabled: true, policy: "open" },
