...
export function applyCliProfileEnv(params: {
  ...
  env.MRHAMMADCLAW_PROFILE = profile;

  const stateDir =
    env.MRHAMMADCLAW_STATE_DIR?.trim() || resolveProfileStateDir(profile, env, homedir);
  if (!env.MRHAMMADCLAW_STATE_DIR?.trim()) {
    env.MRHAMMADCLAW_STATE_DIR = stateDir;
  }

  if (!env.MRHAMMADCLAW_CONFIG_PATH?.trim()) {
    env.MRHAMMADCLAW_CONFIG_PATH = path.join(stateDir, "mrhammadclaw.json");
  }

  if (profile === "dev" && !env.MRHAMMADCLAW_GATEWAY_PORT?.trim()) {
    env.MRHAMMADCLAW_GATEWAY_PORT = "19001";
  }
}
