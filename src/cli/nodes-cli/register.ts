    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["mrhammadclaw nodes status", "List known nodes with live status."],
          ["mrhammadclaw nodes pairing pending", "Show pending node pairing requests."],
          ['mrhammadclaw nodes run --node <id> --raw "uname -a"', "Run a shell command on a node."],
          ["mrhammadclaw nodes camera snap --node <id>", "Capture a photo from a node camera."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/nodes", "docs.mrhammadclaw.ai/cli/nodes")}\n`,
    );
