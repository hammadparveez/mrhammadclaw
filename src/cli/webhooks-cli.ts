...
export function registerWebhooksCli(program: Command) {
  const webhooks = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/webhooks", "docs.mrhammadclaw.ai/cli/webhooks")}\n`,
    );
  ...
  gmail
    .command("setup")
    .description("Configure Gmail watch + Pub/Sub + MrHammadClaw hooks")
    ...
    .option("--hook-url <url>", "MrHammadClaw hook URL")
    .option("--hook-token <token>", "MrHammadClaw hook token")
