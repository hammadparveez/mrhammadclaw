...
import { resolveMrHammadClawPackageRoot } from "../../infra/mrhammadclaw-root.js";
...
const MRHAMMADCLAW_REPO_URL = "https://github.com/hammadparveez/mrhammadclaw.git";
const MAX_LOG_CHARS = 8000;

export const DEFAULT_PACKAGE_NAME = "mrhammadclaw";
...
export function normalizeTag(value?: string | null): string | null {
  ...
  if (trimmed.startsWith("mrhammadclaw@")) {
    return trimmed.slice("mrhammadclaw@".length);
  }
...
export function resolveGitInstallDir(): string {
  const override = process.env.MRHAMMADCLAW_GIT_DIR?.trim();
  if (override) {
...
export async function resolveUpdateRoot(): Promise<string> {
  return (
    (await resolveMrHammadClawPackageRoot({
...
export async function ensureGitCheckout(params: {
  ...
  if (!dirExists) {
    return await runUpdateStep({
      name: "git clone",
      argv: ["git", "clone", MRHAMMADCLAW_REPO_URL, params.dir],
      timeoutMs: params.timeoutMs,
  ...
  if (!(await isGitCheckout(params.dir))) {
    ...
    if (!empty) {
      throw new Error(
        `MRHAMMADCLAW_GIT_DIR points at a non-git directory: ${params.dir}. Set MRHAMMADCLAW_GIT_DIR to an empty folder or an mrhammadclaw checkout.`,
      );
    }
  ...
  if (!(await isCorePackage(params.dir))) {
    throw new Error(`MRHAMMADCLAW_GIT_DIR does not look like a core checkout: ${params.dir}.`);
  }
...
export async function tryWriteCompletionCache(root: string, jsonMode: boolean): Promise<void> {
  const binPath = path.join(root, "mrhammadclaw.mjs");
