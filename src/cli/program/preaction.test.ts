...
beforeEach(() => {
  ...
  originalHideBanner = process.env.MRHAMMADCLAW_HIDE_BANNER;
  delete process.env.NODE_NO_WARNINGS;
  delete process.env.MRHAMMADCLAW_HIDE_BANNER;
});
...
afterEach(() => {
  ...
  if (originalHideBanner === undefined) {
    ...
    process.env.MRHAMMADCLAW_HIDE_BANNER = originalHideBanner;
  }
});

describe("registerPreActionHooks", () => {
  function buildProgram() {
    const program = new Command().name("mrhammadclaw");
  ...
  it("emits banner, resolves config, and enables verbose from --debug", async () => {
    await runCommand({
      parseArgv: ["status"],
      processArgv: ["node", "mrhammadclaw", "status", "--debug"],
    });
    ...
    expect(process.title).toBe("mrhammadclaw-status");
  });

  it("loads plugin registry for plugin-required commands", async () => {
    await runCommand({
      parseArgv: ["message", "send"],
      processArgv: ["node", "mrhammadclaw", "message", "send"],
    });
  ...
  it("loads plugin registry for configure command", async () => {
    await runCommand({
      parseArgv: ["configure"],
      processArgv: ["node", "mrhammadclaw", "configure"],
    });
  ...
  it("loads plugin registry for onboard command", async () => {
    await runCommand({
      parseArgv: ["onboard"],
      processArgv: ["node", "mrhammadclaw", "onboard"],
    });
  ...
  it("loads plugin registry for agents command", async () => {
    await runCommand({
      parseArgv: ["agents"],
      processArgv: ["node", "mrhammadclaw", "agents"],
    });
  ...
  it("skips config guard for doctor, completion, and secrets commands", async () => {
    await runCommand({
      parseArgv: ["doctor"],
      processArgv: ["node", "mrhammadclaw", "doctor"],
    });
    await runCommand({
      parseArgv: ["completion"],
      processArgv: ["node", "mrhammadclaw", "completion"],
    });
    await runCommand({
      parseArgv: ["secrets"],
      processArgv: ["node", "mrhammadclaw", "secrets"],
    });
  ...
  it("skips preaction work when argv indicates help/version", async () => {
    await runCommand({
      parseArgv: ["status"],
      processArgv: ["node", "mrhammadclaw", "--version"],
    });
  ...
  it("hides banner when MRHAMMADCLAW_HIDE_BANNER is truthy", async () => {
    process.env.MRHAMMADCLAW_HIDE_BANNER = "1";
    await runCommand({
      parseArgv: ["status"],
      processArgv: ["node", "mrhammadclaw", "status"],
    });
