...
describe("command-registry", () => {
  ...
  it("registers doctor placeholder for doctor primary command", () => {
    const program = createProgram();
    registerCoreCliCommands(program, testProgramContext, ["node", "mrhammadclaw", "doctor"]);
  ...
  it("registers grouped core entry placeholders without duplicate command errors", async () => {
    const program = createProgram();
    registerCoreCliCommands(program, testProgramContext, ["node", "mrhammadclaw", "vitest"]);
    program.exitOverride();
    await withProcessArgv(["node", "mrhammadclaw", "status"], async () => {
      await program.parseAsync(["node", "mrhammadclaw", "status"]);
    });
