...
const shouldRegisterPrimaryOnly = (argv: string[]) => {
  if (isTruthyEnvValue(process.env.MRHAMMADCLAW_DISABLE_LAZY_SUBCOMMANDS)) {
    return false;
  }
...
const shouldEagerRegisterSubcommands = (_argv: string[]) => {
  return isTruthyEnvValue(process.env.MRHAMMADCLAW_DISABLE_LAZY_SUBCOMMANDS);
};

const loadConfig = async (): Promise<MrHammadClawConfig> => {
...
const entries: SubCliEntry[] = [
  ...
  {
    name: "docs",
    description: "Search the live MrHammadClaw docs",
    hasSubcommands: false,
    register: async (program) => {
