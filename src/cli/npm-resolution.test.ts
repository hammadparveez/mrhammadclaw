...
describe("npm-resolution helpers", () => {
  it("keeps original spec when pin is disabled", () => {
    const result = resolvePinnedNpmSpec({
      rawSpec: "@mrhammadclaw/plugin-alpha@latest",
      pin: false,
      resolvedSpec: "@mrhammadclaw/plugin-alpha@1.2.3",
    });
    expect(result).toEqual({
      recordSpec: "@mrhammadclaw/plugin-alpha@latest",
    });
  });
  ...
  it("returns pinned spec notice when resolved spec is available", () => {
    ...
    expect(result).toEqual({
      recordSpec: "@mrhammadclaw/plugin-alpha@1.2.3",
      pinNotice: "Pinned npm install record to @mrhammadclaw/plugin-alpha@1.2.3.",
    });
  });
  ...
  it("maps npm resolution metadata to install fields", () => {
    expect(
      mapNpmResolutionMetadata({
        name: "@mrhammadclaw/plugin-alpha",
        version: "1.2.3",
        resolvedSpec: "@mrhammadclaw/plugin-alpha@1.2.3",
      ...
      resolvedName: "@mrhammadclaw/plugin-alpha",
  ...
  it("builds common npm install record fields", () => {
    expect(
      buildNpmInstallRecordFields({
        spec: "@mrhammadclaw/plugin-alpha@1.2.3",
        installPath: "/tmp/mrhammadclaw/extensions/alpha",
        version: "1.2.3",
        resolution: {
          name: "@mrhammadclaw/plugin-alpha",
          version: "1.2.3",
  ...
  it("resolves pinned install record and emits pin notice", () => {
    ...
    expect(logs).toEqual(["Pinned npm install record to @mrhammadclaw/plugin-alpha@1.2.3."]);
  ...
  it("resolves pinned install record for CLI and formats warning output", () => {
    ...
    const record = resolvePinnedNpmInstallRecordForCli(
      "@mrhammadclaw/plugin-alpha@latest",
      true,
      "/tmp/mrhammadclaw/extensions/alpha",
      "1.2.3",
      undefined,
      (message) => logs.push(message),
    ...
    expect(record).toEqual({
      source: "npm",
      spec: "@mrhammadclaw/plugin-alpha@latest",
      installPath: "/tmp/mrhammadclaw/extensions/alpha",
      version: "1.2.3",
