...
export function registerSecurityCli(program: Command) {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["mrhammadclaw security audit", "Run a local security audit."],
          ["mrhammadclaw security audit --deep", "Include best-effort live Gateway probe checks."],
          [
            "mrhammadclaw security audit --fix",
            "Apply safe remediations and file-permission fixes.",
          ],
          ["mrhammadclaw security audit --json", "Output machine-readable JSON."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/security", "docs.mrhammadclaw.ai/cli/security")}\n`,
    );
  ...
  security
    ...
    .action(async (opts: SecurityAuditOptions) => {
      ...
      const lines: string[] = [];
      lines.push(heading("MrHammadClaw security audit"));
      lines.push(muted(`Summary: ${formatSummary(report.summary)}`));
      lines.push(muted(`Run deeper: ${formatCliCommand("mrhammadclaw security audit --deep")}`));

      if (opts.fix) {
        lines.push(muted(`Fix: ${formatCliCommand("mrhammadclaw security audit --fix")}`));
        if (!fixResult) {
