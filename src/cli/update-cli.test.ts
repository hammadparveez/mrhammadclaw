...
import type { MrHammadClawConfig, ConfigFileSnapshot } from "../config/types.mrhammadclaw.js";
...
vi.mock("../infra/mrhammadclaw-root.js", () => ({
  resolveMrHammadClawPackageRoot: vi.fn(),
}));
...
const { resolveMrHammadClawPackageRoot } = await import("../infra/mrhammadclaw-root.js");
...
describe("update-cli", () => {
  const fixtureRoot = "/tmp/mrhammadclaw-update-tests";
  let fixtureCount = 0;
  ...
  const baseConfig = {} as MrHammadClawConfig;
  const baseSnapshot: ConfigFileSnapshot = {
    path: "/tmp/mrhammadclaw-config.json",
    exists: true,
    raw: "{}",
    parsed: {},
    resolved: baseConfig,
  ...
  const mockPackageInstallStatus = (root: string) => {
    vi.mocked(resolveMrHammadClawPackageRoot).mockResolvedValue(root);
  ...
  const setupNonInteractiveDowngrade = async () => {
    const tempDir = createCaseDir("mrhammadclaw-update");
    setTty(false);
  ...
  beforeEach(() => {
    ...
    vi.mocked(resolveMrHammadClawPackageRoot).mockClear();
    ...
    vi.mocked(resolveMrHammadClawPackageRoot).mockResolvedValue(process.cwd());
    ...
    readPackageName.mockResolvedValue("mrhammadclaw");
    ...
    prepareRestartScript.mockResolvedValue("/tmp/mrhammadclaw-restart-test.sh");
    runRestartScript.mockResolvedValue(undefined);
    inspectPortUsage.mockResolvedValue({
      port: 18789,
      status: "busy",
      listeners: [{ pid: 4242, command: "mrhammadclaw-gateway" }],
      hints: [],
    });
  ...
  it("updateStatusCommand prints table output", async () => {
    ...
    expect(logs.join("\n")).toContain("MrHammadClaw update status");
  });
    ...
    },
    {
      ...
      prepare: async () => {
        vi.mocked(readConfigFileSnapshot).mockResolvedValue({
          ...baseSnapshot,
          config: { update: { channel: "beta" } } as MrHammadClawConfig,
        });
      },
  ...
  it("honors --tag override", async () => {
    ...
    vi.mocked(resolveMrHammadClawPackageRoot).mockResolvedValue(tempDir);
    vi.mocked(runGatewayUpdate).mockResolvedValue(
      makeOkUpdateResult({
        mode: "npm",
      }),
  ...
  it("updateCommand refreshes service env from updated install root when available", async () => {
    const root = createCaseDir("mrhammadclaw-updated-root");
  ...
  it("updateCommand continues after doctor sub-step and clears update flag", async () => {
    ...
    try {
      await withEnvAsync({ MRHAMMADCLAW_UPDATE_IN_PROGRESS: undefined }, async () => {
        vi.mocked(runGatewayUpdate).mockResolvedValue(makeOkUpdateResult());
        vi.mocked(runDaemonRestart).mockResolvedValue(true);
        vi.mocked(doctorCommand).mockResolvedValue(undefined);
        vi.mocked(defaultRuntime.log).mockClear();
        ...
        expect(process.env.MRHAMMADCLAW_UPDATE_IN_PROGRESS).toBeUndefined();
  ...
  it("updateWizardCommand offers dev checkout and forwards selections", async () => {
    const tempDir = createCaseDir("mrhammadclaw-update-wizard");
    await withEnvAsync({ MRHAMMADCLAW_GIT_DIR: tempDir }, async () => {
      setTty(true);
