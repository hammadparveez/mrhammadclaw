...
export function registerUpdateCli(program: Command) {
  const update = program
    .command("update")
    .description("Update MrHammadClaw and inspect update channel status")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["mrhammadclaw update", "Update a source checkout (git)"],
        ["mrhammadclaw update --channel beta", "Switch to beta channel (git + npm)"],
        ["mrhammadclaw update --channel dev", "Switch to dev channel (git + npm)"],
        ["mrhammadclaw update --tag beta", "One-off update to a dist-tag or version"],
        ["mrhammadclaw update --dry-run", "Preview actions without changing anything"],
        ["mrhammadclaw update --no-restart", "Update without restarting the service"],
        ["mrhammadclaw update --json", "Output result as JSON"],
        ["mrhammadclaw update --yes", "Non-interactive (accept downgrade prompts)"],
        ["mrhammadclaw update wizard", "Interactive update wizard"],
        ["mrhammadclaw --update", "Shorthand for mrhammadclaw update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run mrhammadclaw update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.mrhammadclaw.ai/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["mrhammadclaw update status", "Show channel + version status."],
          ["mrhammadclaw update status --json", "JSON output."],
          ["mrhammadclaw update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.mrhammadclaw.ai/cli/update")}`,
    )
    .action(async (opts, command) => {
      try {
