...
import type { ConfigFileSnapshot, MrHammadClawConfig } from "../config/types.js";

/**
 * Test for issue #6070:
 * `mrhammadclaw config set/unset` must update snapshot.resolved (user config after $include/${ENV},
 * but before runtime defaults), so runtime defaults don't leak into the written config.
 */

const mockReadConfigFileSnapshot = vi.fn<() => Promise<ConfigFileSnapshot>>();
const mockWriteConfigFile = vi.fn<
  (cfg: MrHammadClawConfig, options?: { unsetPaths?: string[][] }) => Promise<void>
>(async () => {});

vi.mock("../config/config.js", () => ({
  readConfigFileSnapshot: () => mockReadConfigFileSnapshot(),
  writeConfigFile: (cfg: MrHammadClawConfig, options?: { unsetPaths?: string[][] }) =>
...
function setSnapshot(resolved: MrHammadClawConfig, config: MrHammadClawConfig) {
...
describe("config cli", () => {
  ...
  describe("config set - issue #6070", () => {
    it("preserves existing config keys when setting a new value", async () => {
      const resolved: MrHammadClawConfig = {
        agents: {
      ...
      const runtimeMerged: MrHammadClawConfig = {
        ...resolved,
        agents: {
          ...resolved.agents,
          defaults: {
  ...
  describe("config get", () => {
    it("redacts sensitive values", async () => {
      ...
      expect(mockLog).toHaveBeenCalledWith("__MRHAMMADCLAW_REDACTED__");
    });
  });

  describe("config set parsing flags", () => {
    it("falls back to raw string when parsing fails and strict mode is off", async () => {
      const resolved: MrHammadClawConfig = { gateway: { port: 18789 } };
      setSnapshot(resolved, resolved);
