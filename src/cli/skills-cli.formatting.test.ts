...
describe("skills-cli (e2e)", () => {
  ...
  beforeAll(() => {
    ...
    tempWorkspaceDir = fs.mkdtempSync(path.join(os.tmpdir(), "mrhammadclaw-skills-test-"));
    tempBundledDir = fs.mkdtempSync(path.join(os.tmpdir(), "mrhammadclaw-bundled-skills-test-"));
    process.env.MRHAMMADCLAW_BUNDLED_SKILLS_DIR = tempBundledDir;
  });
