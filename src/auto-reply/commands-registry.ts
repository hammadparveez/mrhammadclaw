...
export function isCommandEnabled(cfg: MrHammadClawConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: MrHammadClawConfig): {
  provider: string;
  model: string;
} {
  const resolved = resolveConfiguredModelRef({
    cfg: cfg ?? ({} as MrHammadClawConfig),
...
export function getCommandDetection(_cfg?: MrHammadClawConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: MrHammadClawConfig) {
