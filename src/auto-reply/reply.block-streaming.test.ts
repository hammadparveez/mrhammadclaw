...
function createReplyConfig(home: string, streamMode?: "block"): MrHammadClawConfig {
  return {
    agents: {
      defaults: {
...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeHarness("mrhammadclaw-stream-", async (home) => {
...
describe("block streaming", () => {
  beforeEach(() => {
    vi.stubEnv("MRHAMMADCLAW_TEST_FAST", "1");
