...
describe("abort detection", () => {
  ...
  async function createAbortConfig(params?: {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "mrhammadclaw-abort-"));
  ...
  it("isAbortRequestText aligns abort command semantics", () => {
    ...
    expect(isAbortRequestText("stop mrhammadclaw!!!")).toBe(true);
    ...
    expect(isAbortRequestText("/stop@mrhammadclaw_bot", { botUsername: "mrhammadclaw_bot" })).toBe(
      true,
    );
    expect(isAbortRequestText("/Stop@mrhammadclaw_bot", { botUsername: "mrhammadclaw_bot" })).toBe(
      true,
