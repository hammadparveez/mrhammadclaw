...
describe("matchesMentionWithExplicit", () => {
  const mentionRegexes = [/\bmrhammadclaw\b/i];
  ...
  it("resolves known variables, aliases, and case-insensitive tokens", () => {
    const cases = [
      ...
      },
      {
        ...
        values: { identityName: "MrHammadClaw" },
        expected: "[MrHammadClaw]",
      },
      {
      ...
      },
      {
        ...
        values: {
          identityName: "MrHammadClaw",
          provider: "anthropic",
          model: "claude-opus-4-5",
          thinkingLevel: "high",
        },
        expected: "[MrHammadClaw] anthropic/claude-opus-4-5 (think:high)",
      },
    ] as const;
    for (const testCase of cases) {
