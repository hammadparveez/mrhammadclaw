...
export function resolveConfiguredAcpBackendId(cfg: MrHammadClawConfig): string {
  return cfg.acp?.backend?.trim() || "acpx";
}

export function resolveAcpInstallCommandHint(cfg: MrHammadClawConfig): string {
  ...
  if (backendId === "acpx") {
    ...
    if (existsSync(localPath)) {
      return `mrhammadclaw plugins install ${localPath}`;
    }
    return "mrhammadclaw plugins install @mrhammadclaw/acpx";
  }
