...
describe("memory index", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "mrhammadclaw-mem-fixtures-"));
  ...
  beforeEach(async () => {
    ...
    vi.stubEnv("MRHAMMADCLAW_TEST_MEMORY_UNSAFE_REINDEX", "1");
    embedBatchCalls = 0;
  ...
  it("reindexes sessions when source config adds sessions to an existing index", async () => {
    ...
    const previousStateDir = process.env.MRHAMMADCLAW_STATE_DIR;
    ...
    try {
      ...
      if (previousStateDir === undefined) {
        ...
        process.env.MRHAMMADCLAW_STATE_DIR = previousStateDir;
      }
