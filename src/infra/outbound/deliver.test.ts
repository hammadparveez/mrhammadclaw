...
import { resolvePreferredMrHammadClawTmpDir } from "../tmp-mrhammadclaw-dir.js";
...
const telegramChunkConfig: MrHammadClawConfig = {
  channels: { telegram: { botToken: "tok-1", textChunkLimit: 2 } },
};

const whatsappChunkConfig: MrHammadClawConfig = {
...
describe("deliverOutboundPayloads", () => {
  ...
  it("includes MrHammadClaw tmp root in telegram mediaLocalRoots", async () => {
    const sendTelegram = vi.fn().mockResolvedValue({ messageId: "m1", chatId: "c1" });
    ...
    expect(sendTelegram).toHaveBeenCalledWith(
      ...
      "hi",
      expect.objectContaining({
        mediaLocalRoots: expect.arrayContaining([resolvePreferredMrHammadClawTmpDir()]),
      }),
    );
  });

  it("includes MrHammadClaw tmp root in signal mediaLocalRoots", async () => {
  ...
  it("includes MrHammadClaw tmp root in whatsapp mediaLocalRoots", async () => {
  ...
  it("includes MrHammadClaw tmp root in imessage mediaLocalRoots", async () => {
  ...
  it("uses signal media maxBytes from config", async () => {
    const sendSignal = vi.fn().mockResolvedValue({ messageId: "s1", timestamp: 123 });
    const cfg: MrHammadClawConfig = { channels: { signal: { mediaMaxMb: 2 } } };
