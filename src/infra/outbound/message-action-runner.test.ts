...
describe("runMessageAction sandboxed media validation", () => {
  ...
  it("allows media paths under preferred MrHammadClaw tmp root", async () => {
    const tmpRoot = resolvePreferredMrHammadClawTmpDir();
    ...
    try {
      ...
      const hostTmpOutsideMrHammadClaw = path.join(
        os.tmpdir(),
        "outside-mrhammadclaw",
        "test-media.png",
      );
      await expect(
        runMessageAction({
          ...
          params: {
            channel: "slack",
            target: "#C12345678",
            media: hostTmpOutsideMrHammadClaw,
            message: "",
          },
          sandboxRoot: sandboxDir,
          dryRun: true,
