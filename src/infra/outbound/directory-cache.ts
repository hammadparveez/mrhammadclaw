...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: MrHammadClawConfig | null = null;
  ...
  get(key: string, cfg: MrHammadClawConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: MrHammadClawConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  clear(cfg?: MrHammadClawConfig): void {
    this.cache.clear();
    if (cfg) {
  ...
  private resetIfConfigChanged(cfg: MrHammadClawConfig): void {
