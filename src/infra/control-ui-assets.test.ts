...
vi.mock("./mrhammadclaw-root.js", () => ({
  resolveMrHammadClawPackageRoot: vi.fn(async () => null),
  resolveMrHammadClawPackageRootSync: vi.fn(() => null),
}));
...
let mrhammadclawRoot: typeof import("./mrhammadclaw-root.js");
...
describe("control UI assets helpers (fs-mocked)", () => {
  beforeAll(async () => {
    ...
    mrhammadclawRoot = await import("./mrhammadclaw-root.js");
  });
  ...
  it("uses resolveMrHammadClawPackageRoot when available", async () => {
    const pkgRoot = abs("fixtures/mrhammadclaw");
    (
      mrhammadclawRoot.resolveMrHammadClawPackageRoot as unknown as ReturnType<typeof vi.fn>
    ).mockResolvedValueOnce(pkgRoot);

    await expect(resolveControlUiDistIndexPath(abs("fixtures/bin/mrhammadclaw"))).resolves.toBe(
  ...
  it("falls back to package.json name matching when root resolution fails", async () => {
    ...
    setFile(path.join(root, "package.json"), JSON.stringify({ name: "mrhammadclaw" }));
    setFile(path.join(root, "dist", "control-ui", "index.html"), "<html></html>\n");

    await expect(resolveControlUiDistIndexPath(path.join(root, "mrhammadclaw.mjs"))).resolves.toBe(
      path.join(root, "dist", "control-ui", "index.html"),
    );
  });

  it("returns null when fallback package name does not match", async () => {
    const root = abs("fixtures/not-mrhammadclaw");
  ...
  it("resolves control-ui root for dist bundle argv1 and moduleUrl candidates", async () => {
    const pkgRoot = abs("fixtures/mrhammadclaw-bundle");
    (
      mrhammadclawRoot.resolveMrHammadClawPackageRootSync as unknown as ReturnType<typeof vi.fn>
