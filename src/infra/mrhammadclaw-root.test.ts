...
const VITEST_FS_BASE = path.join(path.parse(process.cwd()).root, "__mrhammadclaw_vitest__");
const FIXTURE_BASE = path.join(VITEST_FS_BASE, "mrhammadclaw-root");
...
describe("resolveMrHammadClawPackageRoot", () => {
  let resolveMrHammadClawPackageRoot: typeof import("./mrhammadclaw-root.js").resolveMrHammadClawPackageRoot;
  let resolveMrHammadClawPackageRootSync: typeof import("./mrhammadclaw-root.js").resolveMrHammadClawPackageRootSync;

  beforeAll(async () => {
    ({ resolveMrHammadClawPackageRoot, resolveMrHammadClawPackageRootSync } =
      await import("./mrhammadclaw-root.js"));
  });
  ...
  it("resolves package root from .bin argv1", async () => {
    ...
    const argv1 = path.join(project, "node_modules", ".bin", "mrhammadclaw");
    const pkgRoot = path.join(project, "node_modules", "mrhammadclaw");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "mrhammadclaw" }));

    expect(resolveMrHammadClawPackageRootSync({ argv1 })).toBe(pkgRoot);
  });

  it("resolves package root via symlinked argv1", async () => {
    const project = fx("symlink-scenario");
    const bin = path.join(project, "bin", "mrhammadclaw");
    const realPkg = path.join(project, "real-pkg");
    state.realpaths.set(abs(bin), abs(path.join(realPkg, "mrhammadclaw.mjs")));
    setFile(path.join(realPkg, "package.json"), JSON.stringify({ name: "mrhammadclaw" }));

    expect(resolveMrHammadClawPackageRootSync({ argv1: bin })).toBe(realPkg);
  });
  ...
  it("prefers moduleUrl candidates", async () => {
    ...
    expect(resolveMrHammadClawPackageRootSync({ moduleUrl })).toBe(pkgRoot);
  });

  it("returns null for non-mrhammadclaw package roots", async () => {
    const pkgRoot = fx("not-mrhammadclaw");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "not-mrhammadclaw" }));

    expect(resolveMrHammadClawPackageRootSync({ cwd: pkgRoot })).toBeNull();
  });
  ...
  it("async resolver matches sync behavior", async () => {
    ...
    await expect(resolveMrHammadClawPackageRoot({ cwd: pkgRoot })).resolves.toBe(pkgRoot);
  });

  it("async resolver returns null when no package roots exist", async () => {
    await expect(resolveMrHammadClawPackageRoot({ cwd: fx("missing") })).resolves.toBeNull();
  });
});
