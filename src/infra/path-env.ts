...
type EnsureMrHammadClawPathOpts = {
  execPath?: string;
  cwd?: string;
  homeDir?: string;
...
function candidateBinDirs(opts: EnsureMrHammadClawPathOpts): {
  prepend: string[];
  append: string[];
} {
  ...
  // Bundled macOS app: `mrhammadclaw` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "mrhammadclaw");
  ...
  const allowProjectLocalBin =
    ...
    isTruthyEnvValue(process.env.MRHAMMADCLAW_ALLOW_PROJECT_LOCAL_BIN);
  if (allowProjectLocalBin) {
    const localBinDir = path.join(cwd, "node_modules", ".bin");
    if (isExecutable(path.join(localBinDir, "mrhammadclaw"))) {
...
/**
 * Best-effort PATH bootstrap so skills that require the `mrhammadclaw` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureMrHammadClawCliOnPath(opts: EnsureMrHammadClawPathOpts = {}) {
  if (isTruthyEnvValue(process.env.MRHAMMADCLAW_PATH_BOOTSTRAPPED)) {
    return;
  }
