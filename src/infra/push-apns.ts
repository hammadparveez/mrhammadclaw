...
export async function resolveApnsAuthConfigFromEnv(
  ...
  const teamId = normalizeNonEmptyString(env.MRHAMMADCLAW_APNS_TEAM_ID);
  const keyId = normalizeNonEmptyString(env.MRHAMMADCLAW_APNS_KEY_ID);
  if (!teamId || !keyId) {
    return {
      ok: false,
      error: "APNs auth missing: set MRHAMMADCLAW_APNS_TEAM_ID and MRHAMMADCLAW_APNS_KEY_ID",
    };
  }

  const inlineKeyRaw =
    normalizeNonEmptyString(env.MRHAMMADCLAW_APNS_PRIVATE_KEY_P8) ??
    normalizeNonEmptyString(env.MRHAMMADCLAW_APNS_PRIVATE_KEY);
  if (inlineKeyRaw) {
    return {
      ok: true,
      value: {
  ...
  const keyPath = normalizeNonEmptyString(env.MRHAMMADCLAW_APNS_PRIVATE_KEY_PATH);
  if (!keyPath) {
    return {
      ok: false,
      error:
        "APNs private key missing: set MRHAMMADCLAW_APNS_PRIVATE_KEY_P8 or MRHAMMADCLAW_APNS_PRIVATE_KEY_PATH",
    };
  }
  try {
    ...
    return {
      ok: false,
      error: `failed reading MRHAMMADCLAW_APNS_PRIVATE_KEY_PATH (${keyPath}): ${message}`,
    };
  }
}
...
function createMrHammadClawPushMetadata(params: {
...
export async function sendApnsAlert(params: {
  ...
  const payload = {
    ...
    mrhammadclaw: createMrHammadClawPushMetadata({
      kind: "push.test",
      nodeId: params.nodeId,
    }),
  };
