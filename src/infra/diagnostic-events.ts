...
function getDiagnosticEventsState(): DiagnosticEventsGlobalState {
  const globalStore = globalThis as typeof globalThis & {
    __mrhammadclawDiagnosticEventsState?: DiagnosticEventsGlobalState;
  };
  if (!globalStore.__mrhammadclawDiagnosticEventsState) {
    globalStore.__mrhammadclawDiagnosticEventsState = {
      seq: 0,
      listeners: new Set<(evt: DiagnosticEventPayload) => void>(),
      dispatchDepth: 0,
    };
  }
  return globalStore.__mrhammadclawDiagnosticEventsState;
}

export function isDiagnosticsEnabled(config?: MrHammadClawConfig): boolean {
