...
let ensureMrHammadClawCliOnPath: typeof import("./path-env.js").ensureMrHammadClawCliOnPath;

describe("ensureMrHammadClawCliOnPath", () => {
  const envKeys = [
    "PATH",
    "MRHAMMADCLAW_PATH_BOOTSTRAPPED",
    "MRHAMMADCLAW_ALLOW_PROJECT_LOCAL_BIN",
    "MISE_DATA_DIR",
    "HOMEBREW_PREFIX",
    "HOMEBREW_BREW_FILE",
    "XDG_BIN_HOME",
  ...
  beforeAll(async () => {
    ({ ensureMrHammadClawCliOnPath } = await import("./path-env.js"));
  });
  ...
  it("prepends the bundled app bin dir when a sibling mrhammadclaw exists", () => {
    const tmp = abs("/tmp/mrhammadclaw-path/case-bundled");
    const appBinDir = path.join(tmp, "AppBin");
    const cliPath = path.join(appBinDir, "mrhammadclaw");
    setDir(tmp);
    setDir(appBinDir);
    setExe(cliPath);

    process.env.PATH = "/usr/bin";
    delete process.env.MRHAMMADCLAW_PATH_BOOTSTRAPPED;

    ensureMrHammadClawCliOnPath({
      execPath: cliPath,
      cwd: tmp,
      homeDir: tmp,
      platform: "darwin",
  ...
  it("is idempotent", () => {
    ...
    process.env.MRHAMMADCLAW_PATH_BOOTSTRAPPED = "1";
    ensureMrHammadClawCliOnPath({
  ...
  it("prepends mise shims when available", () => {
    const tmp = abs("/tmp/mrhammadclaw-path/case-mise");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "mrhammadclaw");
    setDir(tmp);
    setDir(appBinDir);
    setExe(appCli);
  ...
  it("only appends project-local node_modules/.bin when explicitly enabled", () => {
    const tmp = abs("/tmp/mrhammadclaw-path/case-project-local");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "mrhammadclaw");
    setDir(tmp);
    setDir(appBinDir);
    ...
    const localCli = path.join(localBinDir, "mrhammadclaw");
  ...
  it("prepends Linuxbrew dirs when present", () => {
    const tmp = abs("/tmp/mrhammadclaw-path/case-linuxbrew");
