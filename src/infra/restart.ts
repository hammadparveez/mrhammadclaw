...
export function triggerMrHammadClawRestart(): RestartAttempt {
  ...
  if (process.platform !== "darwin") {
    if (process.platform === "linux") {
      const unit = normalizeSystemdUnit(
        process.env.MRHAMMADCLAW_SYSTEMD_UNIT,
        process.env.MRHAMMADCLAW_PROFILE,
      );
  ...
  const label =
    process.env.MRHAMMADCLAW_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.MRHAMMADCLAW_PROFILE);
