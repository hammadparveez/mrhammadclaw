...
describe("fs-safe", () => {
  it("reads a local file safely", async () => {
    const dir = await tempDirs.make("mrhammadclaw-fs-safe-");
  ...
  it("blocks traversal outside root", async () => {
    const root = await tempDirs.make("mrhammadclaw-fs-safe-root-");
    const outside = await tempDirs.make("mrhammadclaw-fs-safe-outside-");
