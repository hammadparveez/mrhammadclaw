...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.MRHAMMADCLAW_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*mrhammadclaw-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "mrhammadclaw";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "mrhammadclaw");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "mrhammadclaw-gw");
  ...
  records.push(`_mrhammadclaw-gw._tcp IN PTR ${instanceLabel}._mrhammadclaw-gw._tcp`);
  records.push(
    `${instanceLabel}._mrhammadclaw-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`,
  );
  records.push(`${instanceLabel}._mrhammadclaw-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; mrhammadclaw-content-hash: ${contentHash}\n${contentBody}`;
}
