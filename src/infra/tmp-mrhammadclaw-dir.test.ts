...
import {
  POSIX_MRHAMMADCLAW_TMP_DIR,
  resolvePreferredMrHammadClawTmpDir,
} from "./tmp-mrhammadclaw-dir.js";

type TmpDirOptions = NonNullable<Parameters<typeof resolvePreferredMrHammadClawTmpDir>[0]>;

function fallbackTmp(uid = 501) {
  return path.join("/var/fallback", `mrhammadclaw-${uid}`);
}
...
function resolveWithMocks(params: {
  ...
  const wrappedLstatSync = vi.fn((target: string) => {
    if (target === POSIX_MRHAMMADCLAW_TMP_DIR) {
  ...
  const resolved = resolvePreferredMrHammadClawTmpDir({
    accessSync,
...
describe("resolvePreferredMrHammadClawTmpDir", () => {
  it("prefers /tmp/mrhammadclaw when it already exists and is writable", () => {
    ...
    expect(resolved).toBe(POSIX_MRHAMMADCLAW_TMP_DIR);
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("prefers /tmp/mrhammadclaw when it does not exist but /tmp is writable", () => {
    ...
    expect(mkdirSync).toHaveBeenCalledWith(POSIX_MRHAMMADCLAW_TMP_DIR, expect.any(Object));
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("falls back to os.tmpdir()/mrhammadclaw when /tmp/mrhammadclaw is not a directory", () => {
  ...
  it("falls back to os.tmpdir()/mrhammadclaw when /tmp is not writable", () => {
  ...
  it("falls back when /tmp/mrhammadclaw is a symlink", () => {
    const lstatSync = vi.fn(() => ({
      isDirectory: () => true,
      isSymbolicLink: () => true,
      uid: 501,
  ...
  it("falls back when /tmp/mrhammadclaw is not owned by the current user", () => {
    const lstatSync = vi.fn(() => ({
      isDirectory: () => true,
      isSymbolicLink: () => false,
      uid: 0,
  ...
  it("falls back when /tmp/mrhammadclaw is group/other writable", () => {
    const lstatSync = vi.fn(() => ({
      isDirectory: () => true,
      isSymbolicLink: () => false,
      uid: 501,
  ...
  it("throws when fallback path is a symlink", () => {
    ...
    ).toThrow(/Unsafe fallback MrHammadClaw temp dir/);
  });
