...
describe("update-startup", () => {
  ...
  beforeAll(async () => {
    suiteRoot = await fs.mkdtemp(path.join(os.tmpdir(), "mrhammadclaw-update-check-suite-"));
  });
  ...
  beforeEach(async () => {
    ...
    envSnapshot = captureEnv(["MRHAMMADCLAW_STATE_DIR", "NODE_ENV", "VITEST"]);
    process.env.MRHAMMADCLAW_STATE_DIR = tempDir;
    ...
    if (!loaded) {
      ({ resolveMrHammadClawPackageRoot } = await import("./mrhammadclaw-root.js"));
  ...
  function mockPackageUpdateStatus(tag = "latest", version = "2.0.0") {
    vi.mocked(resolveMrHammadClawPackageRoot).mockResolvedValue("/opt/mrhammadclaw");
    vi.mocked(checkUpdateStatus).mockResolvedValue({
      root: "/opt/mrhammadclaw",
      installKind: "package",
      packageManager: "npm",
  ...
  it("uses current runtime + entrypoint for default auto-update command execution", async () => {
    ...
    process.argv = [process.execPath, "/opt/mrhammadclaw/dist/entry.js"];
    try {
      await runGatewayUpdateCheck({
        cfg: {
          update: {
    ...
    expect(runCommandWithTimeout).toHaveBeenCalledWith(
      [
        process.execPath,
        "/opt/mrhammadclaw/dist/entry.js",
        "update",
        "--yes",
        "--channel",
        "beta",
