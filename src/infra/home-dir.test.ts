...
describe("resolveEffectiveHomeDir", () => {
  it("prefers MRHAMMADCLAW_HOME over HOME and USERPROFILE", () => {
    const env = {
  ...
  it("expands MRHAMMADCLAW_HOME when set to ~", () => {
    const env = {
      MRHAMMADCLAW_HOME: "~/svc",
      HOME: "/home/alice",
    } as NodeJS.ProcessEnv;
...
describe("resolveRequiredHomeDir", () => {
  ...
  it("returns a fully resolved path for MRHAMMADCLAW_HOME", () => {
    const result = resolveRequiredHomeDir(
      { MRHAMMADCLAW_HOME: "/custom/home" } as NodeJS.ProcessEnv,
      () => "/fallback",
    );
    expect(result).toBe(path.resolve("/custom/home"));
  });

  it("returns cwd when MRHAMMADCLAW_HOME is tilde-only and no fallback home exists", () => {
    expect(
      resolveRequiredHomeDir({ MRHAMMADCLAW_HOME: "~" } as NodeJS.ProcessEnv, () => {
        throw new Error("no home");
      }),
    ).toBe(process.cwd());
  });
...
describe("expandHomePrefix", () => {
  it("expands tilde using effective home", () => {
    const value = expandHomePrefix("~/x", {
      env: { MRHAMMADCLAW_HOME: "/srv/mrhammadclaw-home" } as NodeJS.ProcessEnv,
    });
    expect(value).toBe(`${path.resolve("/srv/mrhammadclaw-home")}/x`);
  });
