...
const DEFAULT_PACKAGE_NAME = "mrhammadclaw";
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(
        path.join(os.tmpdir(), "mrhammadclaw-update-preflight-"),
      );
    ...
    const doctorEntry = path.join(gitRoot, "mrhammadclaw.mjs");
    ...
    if (!doctorEntryExists) {
      steps.push({
        name: "mrhammadclaw doctor entry",
    ...
    const doctorStep = await runStep(
      step("mrhammadclaw doctor", doctorArgv, gitRoot, { MRHAMMADCLAW_UPDATE_IN_PROGRESS: "1" }),
    );
