...
describe("resolveGatewayConnection", () => {
  ...
  it("uses MRHAMMADCLAW_GATEWAY_TOKEN for local mode", () => {
    loadConfig.mockReturnValue({ gateway: { mode: "local" } });

    withEnv({ MRHAMMADCLAW_GATEWAY_TOKEN: "env-token" }, () => {
      const result = resolveGatewayConnection({});
  ...
  it("prefers MRHAMMADCLAW_GATEWAY_PASSWORD over remote password fallback", () => {
    ...
    withEnv({ MRHAMMADCLAW_GATEWAY_PASSWORD: "env-pass" }, () => {
      const result = resolveGatewayConnection({});
