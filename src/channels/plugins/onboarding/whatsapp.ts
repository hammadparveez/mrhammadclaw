...
function setWhatsAppDmPolicy(cfg: MrHammadClawConfig, dmPolicy: DmPolicy): MrHammadClawConfig {
  return mergeWhatsAppConfig(cfg, { dmPolicy });
}

function setWhatsAppAllowFrom(cfg: MrHammadClawConfig, allowFrom?: string[]): MrHammadClawConfig {
...
async function detectWhatsAppLinked(cfg: MrHammadClawConfig, accountId: string): Promise<boolean> {
...
async function promptWhatsAppOwnerAllowFrom(params: {
  ...
  await prompter.note(
    "We need the sender/owner number so MrHammadClaw can allowlist you.",
    "WhatsApp number",
  );
...
async function promptWhatsAppAllowFrom(
  ...
  const phoneMode = await prompter.select({
    ...
    options: [
      ...
      { value: "separate", label: "Separate phone just for MrHammadClaw" },
    ],
  });
...
export const whatsappOnboardingAdapter: ChannelOnboardingAdapter = {
  ...
  configure: async ({
    ...
    if (wantsLink) {
      ...
      await prompter.note(
        `Run \`${formatCliCommand("mrhammadclaw channels login")}\` later to link WhatsApp.`,
        "WhatsApp",
      );
    }
