...
async function runConfigureChannelAccess<TResolved>(params: {
  ...
  label?: string;
  placeholder?: string;
  setPolicy: (cfg: MrHammadClawConfig, policy: ChannelAccessPolicy) => MrHammadClawConfig;
  resolveAllowlist: (params: { cfg: MrHammadClawConfig; entries: string[] }) => Promise<TResolved>;
  applyAllowlist: (params: { cfg: MrHammadClawConfig; resolved: TResolved }) => MrHammadClawConfig;
}) {
...
describe("configureChannelAccessWithAllowlist", () => {
  it("returns input config when user skips access configuration", async () => {
    ...
    const setPolicy = vi.fn((next: MrHammadClawConfig) => next);
    const resolveAllowlist = vi.fn(async () => [] as string[]);
    const applyAllowlist = vi.fn((params: { cfg: MrHammadClawConfig }) => params.cfg);
  ...
  it("applies non-allowlist policy directly", async () => {
    ...
    const setPolicy = vi.fn(
      (next: MrHammadClawConfig, policy: ChannelAccessPolicy): MrHammadClawConfig => ({
        ...next,
  ...
  it("resolves allowlist entries and applies them after forcing allowlist policy", async () => {
    ...
    const resolveAllowlist = vi.fn(
      async (params: { cfg: MrHammadClawConfig; entries: string[] }) => {
        calls.push("resolve");
    ...
    const applyAllowlist = vi.fn((params: { cfg: MrHammadClawConfig; resolved: string[] }) => {
      calls.push("apply");
