...
describe("channel plugin catalog", () => {
  it("includes Microsoft Teams", () => {
    ...
    expect(entry?.install.npmSpec).toBe("@mrhammadclaw/msteams");
  ...
  it("includes external catalog entries", () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), "mrhammadclaw-catalog-"));
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        entries: [
          {
            name: "@mrhammadclaw/demo-channel",
            mrhammadclaw: {
              channel: {
                id: "demo-channel",
                label: "Demo Channel",
              ...
              install: {
                npmSpec: "@mrhammadclaw/demo-channel",
              },
            },
          },
        ],
...
async function listDirectoryEntriesWithDefaults(listFn: DirectoryListFn, cfg: MrHammadClawConfig) {
  return await listFn({
    cfg,
    accountId: "default",
    query: null,
