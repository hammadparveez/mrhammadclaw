...
describe("message action capability checks", () => {
  ...
  it("aggregates buttons/card support across plugins", () => {
    ...
    expect(supportsChannelMessageButtons({} as MrHammadClawConfig)).toBe(true);
    expect(supportsChannelMessageCards({} as MrHammadClawConfig)).toBe(true);
  });
  ...
  it("checks per-channel capabilities", () => {
    ...
    expect(
      supportsChannelMessageCardsForChannel({ cfg: {} as MrHammadClawConfig, channel: "telegram" }),
    ).toBe(true);
    expect(supportsChannelMessageCardsForChannel({ cfg: {} as MrHammadClawConfig })).toBe(false);
  });
});
