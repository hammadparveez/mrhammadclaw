...
function cfg(accounts?: Record<string, unknown> | null): MrHammadClawConfig {
  if (accounts === null) {
    return { channels: { testchannel: {} } } as unknown as MrHammadClawConfig;
  }
  if (accounts === undefined) {
    return {} as unknown as MrHammadClawConfig;
  }
  return { channels: { testchannel: { accounts } } } as unknown as MrHammadClawConfig;
}

describe("createAccountListHelpers", () => {
  describe("listConfiguredAccountIds", () => {
    it("returns empty for missing config", () => {
      expect(listConfiguredAccountIds({} as MrHammadClawConfig)).toEqual([]);
    });
  ...
  describe("listAccountIds", () => {
    it('returns ["default"] for empty config', () => {
      expect(listAccountIds({} as MrHammadClawConfig)).toEqual(["default"]);
    });
  ...
  describe("resolveDefaultAccountId", () => {
    ...
    it('returns "default" for empty config', () => {
      expect(resolveDefaultAccountId({} as MrHammadClawConfig)).toBe("default");
    });
  });
});
