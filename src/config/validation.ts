...
import type { MrHammadClawConfig, ConfigValidationIssue } from "./types.js";
...
function validateIdentityAvatar(config: MrHammadClawConfig): ConfigValidationIssue[] {
...
export function validateConfigObjectRaw(
  raw: unknown,
): { ok: true; config: MrHammadClawConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = MrHammadClawSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as MrHammadClawConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as MrHammadClawConfig);
  if (avatarIssues.length > 0) {
    return { ok: false, issues: avatarIssues };
  }
  return {
    ok: true,
    config: validated.data as MrHammadClawConfig,
  };
}
