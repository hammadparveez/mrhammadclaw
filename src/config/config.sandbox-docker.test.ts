...
describe("sandbox browser binds config", () => {
  ...
  it("defaults browser network to dedicated sandbox network", () => {
    ...
    expect(resolved.network).toBe("mrhammadclaw-sandbox-browser");
  });

  it("prefers agent browser network over global browser network", () => {
    const resolved = resolveSandboxBrowserConfig({
      scope: "agent",
      globalBrowser: { network: "mrhammadclaw-sandbox-browser-global" },
      agentBrowser: { network: "mrhammadclaw-sandbox-browser-agent" },
    });
    expect(resolved.network).toBe("mrhammadclaw-sandbox-browser-agent");
  });
