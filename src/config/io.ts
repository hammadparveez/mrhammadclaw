...
import type { MrHammadClawConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import {
  validateConfigObjectRawWithPlugins,
  validateConfigObjectWithPlugins,
} from "./validation.js";
import { compareMrHammadClawVersions } from "./version.js";
...
function formatConfigValidationFailure(pathLabel: string, issueMessage: string): string {
  ...
  return [
    ...
    "",
    "Fix with:",
    `  mrhammadclaw config set ${allowPath} '["*"]'`,
    "",
    "Or switch policy:",
    `  mrhammadclaw config set ${policyPath} "pairing"`,
  ].join("\n");
}
...
function unsetPathForWrite(
  root: MrHammadClawConfig,
  pathSegments: string[],
): { changed: boolean; next: MrHammadClawConfig } {
...
function coerceConfig(value: unknown): MrHammadClawConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as MrHammadClawConfig;
}
...
function stampConfigVersion(cfg: MrHammadClawConfig): MrHammadClawConfig {
...
function warnIfConfigFromFuture(
  ...
  const cmp = compareMrHammadClawVersions(VERSION, touched);
  if (cmp === null) {
    return;
  }
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer MrHammadClaw (${touched}); current version is ${VERSION}.`,
    );
  }
}
...
function resolveConfigForRead(
  ...
  if (resolvedIncludes && typeof resolvedIncludes === "object" && "env" in resolvedIncludes) {
    applyConfigEnvVars(resolvedIncludes as MrHammadClawConfig, env);
  }
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): MrHammadClawConfig {
    try {
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as MrHammadClawConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: MrHammadClawConfig, options: ConfigWriteOptions = {}) {
    clearConfigCache();
    ...
    const outputConfigBase =
      envRefMap && changedPaths
        ? (restoreEnvRefsFromMap(cfgToWrite, "", envRefMap, changedPaths) as MrHammadClawConfig)
        : cfgToWrite;
    ...
    const logConfigOverwrite = () => {
      ...
      const shouldLogInVitest = deps.env.MRHAMMADCLAW_TEST_CONFIG_OVERWRITE_LOG === "1";
    ...
    const logConfigWriteAnomalies = () => {
      ...
      const isVitest = deps.env.VITEST === "true";
      const shouldLogInVitest = deps.env.MRHAMMADCLAW_TEST_CONFIG_WRITE_ANOMALY_LOG === "1";
      if (isVitest && !shouldLogInVitest) {
        return;
      }
    ...
    const auditRecordBase = {
      ...
      watchMode: deps.env.MRHAMMADCLAW_WATCH_MODE === "1",
      watchSession:
        typeof deps.env.MRHAMMADCLAW_WATCH_SESSION === "string" &&
        deps.env.MRHAMMADCLAW_WATCH_SESSION.trim().length > 0
          ? deps.env.MRHAMMADCLAW_WATCH_SESSION.trim()
          : null,
      watchCommand:
        typeof deps.env.MRHAMMADCLAW_WATCH_COMMAND === "string" &&
        deps.env.MRHAMMADCLAW_WATCH_COMMAND.trim().length > 0
          ? deps.env.MRHAMMADCLAW_WATCH_COMMAND.trim()
          : null,
      existsBefore: snapshot.exists,
...
// module scope. `MRHAMMADCLAW_CONFIG_PATH` (and friends) are expected to work even
...
let runtimeConfigSnapshot: MrHammadClawConfig | null = null;
let runtimeConfigSourceSnapshot: MrHammadClawConfig | null = null;

function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.MRHAMMADCLAW_CONFIG_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.MRHAMMADCLAW_DISABLE_CONFIG_CACHE?.trim()) {
    return false;
  }
...
export function setRuntimeConfigSnapshot(
  config: MrHammadClawConfig,
  sourceConfig?: MrHammadClawConfig,
): void {
...
export function getRuntimeConfigSnapshot(): MrHammadClawConfig | null {
  return runtimeConfigSnapshot;
}

export function loadConfig(): MrHammadClawConfig {
