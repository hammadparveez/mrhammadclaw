...
import type { ConfigFileSnapshot } from "./types.mrhammadclaw.js";
...
function makeSnapshot<TConfig extends Record<string, unknown>>(
  ...
  return {
    path: "/home/user/.mrhammadclaw/config.json5",
    exists: true,
...
describe("redactConfigSnapshot", () => {
  ...
  it("does not redact passwordFile path fields", () => {
    const snapshot = makeSnapshot({
      channels: {
        irc: {
          passwordFile: "/etc/mrhammadclaw/irc-password.txt",
          nickserv: {
            passwordFile: "/etc/mrhammadclaw/nickserv-password.txt",
    ...
    expect(irc.passwordFile).toBe("/etc/mrhammadclaw/irc-password.txt");
    expect(nickserv.passwordFile).toBe("/etc/mrhammadclaw/nickserv-password.txt");
  ...
  it("contract-covers dynamic catchall/record paths for redact+restore", () => {
    const hints = mapSensitivePaths(MrHammadClawSchema, "", {});
    const snapshot = makeSnapshot({
      env: {
...
describe("realredactConfigSnapshot_real", () => {
  it("main schema redact works (samples)", () => {
    const schema = MrHammadClawSchema.toJSONSchema({
      target: "draft-07",
      unrepresentable: "any",
    });
    schema.title = "MrHammadClawConfig";
    const hints = mapSensitivePaths(MrHammadClawSchema, "", {});
