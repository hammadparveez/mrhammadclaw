  ...
  it("expands tilde for path-ish keys only", async () => {
    await withTempHome(async (home) => {
      const cfg = normalizeConfigPaths({
        ...
        logging: { file: "~/.mrhammadclaw/logs/mrhammadclaw.log" },
        hooks: {
          path: "~/.mrhammadclaw/hooks.json5",
        ...
        channels: {
          telegram: {
            accounts: {
              personal: {
                tokenFile: "~/.mrhammadclaw/telegram.token",
              },
            },
          },
          imessage: {
      ...
      expect(cfg.logging?.file).toBe(path.join(home, ".mrhammadclaw", "logs", "mrhammadclaw.log"));
      expect(cfg.hooks?.path).toBe(path.join(home, ".mrhammadclaw", "hooks.json5"));
      ...
      expect(cfg.channels?.telegram?.accounts?.personal?.tokenFile).toBe(
        path.join(home, ".mrhammadclaw", "telegram.token"),
      );
