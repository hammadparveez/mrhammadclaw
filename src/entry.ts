...
const ENTRY_WRAPPER_PAIRS = [
  { wrapperBasename: "mrhammadclaw.mjs", entryBasename: "entry.js" },
  { wrapperBasename: "mrhammadclaw.mjs", entryBasename: "entry.js" },
  { wrapperBasename: "mrhammadclaw.js", entryBasename: "entry.js" },
] as const;
...
if (
  ...
  function ensureExperimentalWarningSuppressed(): boolean {
    ...
    if (isTruthyEnvValue(process.env.MRHAMMADCLAW_NO_RESPAWN)) {
      return false;
    }
    if (isTruthyEnvValue(process.env.MRHAMMADCLAW_NODE_OPTIONS_READY)) {
      return false;
    }
    ...
    process.env.MRHAMMADCLAW_NODE_OPTIONS_READY = "1";
    ...
    child.once("error", (error) => {
      console.error(
        "[mrhammadclaw] Failed to respawn CLI:",
  ...
  if (!ensureExperimentalWarningSuppressed()) {
    ...
    if (!parsed.ok) {
      ...
      console.error(`[mrhammadclaw] ${parsed.error}`);
      process.exit(2);
    }
    ...
    import("./cli/run-main.js")
      ...
      .catch((error) => {
        console.error(
          "[mrhammadclaw] Failed to start CLI:",
          error instanceof Error ? (error.stack ?? error.message) : error,
        );
        process.exitCode = 1;
      });
