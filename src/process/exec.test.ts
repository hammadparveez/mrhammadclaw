...
describe("runCommandWithTimeout", () => {
  ...
  it("merges custom env with process.env", async () => {
    await withEnvAsync({ MRHAMMADCLAW_BASE_ENV: "base" }, async () => {
      const result = await runCommandWithTimeout(
        [
          process.execPath,
          "-e",
          'process.stdout.write((process.env.MRHAMMADCLAW_BASE_ENV ?? "") + "|" + (process.env.MRHAMMADCLAW_TEST_ENV ?? ""))',
        ],
        {
          timeoutMs: 5_000,
          env: { MRHAMMADCLAW_TEST_ENV: "ok" },
        },
      );
