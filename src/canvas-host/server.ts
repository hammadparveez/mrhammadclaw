...
<title>MrHammadClaw Canvas</title>
<style>
...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      window.webkit.messageHandlers.mrhammadclawCanvasA2UIAction
    );
  const hasAndroid = () =>
    !!(
      (window.mrhammadclawCanvasA2UIAction &&
        typeof window.mrhammadclawCanvasA2UIAction.postMessage === "function")
    );
  const hasHelper = () => typeof window.mrhammadclawSendUserAction === "function";
  statusEl.innerHTML =
    "Bridge: " +
  ...
  window.addEventListener("mrhammadclaw:a2ui-action-status", onStatus);

  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android MrHammadClaw node canvas.");
      return;
    }
    const sendUserAction =
      typeof window.mrhammadclawSendUserAction === "function"
        ? window.mrhammadclawSendUserAction
        : undefined;
...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.MRHAMMADCLAW_SKIP_CANVAS_HOST)) {
    return true;
  }
  if (isTruthyEnvValue(process.env.MRHAMMADCLAW_SKIP_CANVAS_HOST)) {
    return true;
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>MrHammadClaw Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;
