...
describe("canvas host state dir defaults", () => {
  it("uses MRHAMMADCLAW_STATE_DIR for the default canvas root", async () => {
    await withStateDirEnv("mrhammadclaw-canvas-state-", async ({ stateDir }) => {
      ...
      try {
        ...
        expect(indexContents).toContain("MrHammadClaw Canvas");
      } finally {
        await handler.close();
      }
    });
