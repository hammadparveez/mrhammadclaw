...
import { discoverMrHammadClawPlugins } from "./discovery.js";

const tempDirs: string[] = [];

function makeTempDir() {
  const dir = path.join(os.tmpdir(), `mrhammadclaw-plugins-${randomUUID()}`);
  ...
  return await withEnvAsync(
    {
      ...
      MRHAMMADCLAW_BUNDLED_PLUGINS_DIR: "/nonexistent/bundled/plugins",
    },
    fn,
  );
}
...
describe("discoverMrHammadClawPlugins", () => {
  it("discovers global and workspace extensions", async () => {
    ...
    const workspaceExt = path.join(workspaceDir, ".mrhammadclaw", "extensions");
    fs.mkdirSync(workspaceExt, { recursive: true });
    fs.writeFileSync(path.join(workspaceExt, "beta.ts"), "export default function () {}", "utf-8");

    const { candidates } = await withStateDir(stateDir, async () => {
      return discoverMrHammadClawPlugins({ workspaceDir });
    });
  ...
  it("ignores backup and disabled plugin directories in scanned roots", async () => {
    ...
    const { candidates } = await withStateDir(stateDir, async () => {
      return discoverMrHammadClawPlugins({});
    });
  ...
  it("loads package extension packs", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        name: "pack",
        mrhammadclaw: { extensions: ["./src/one.ts", "./src/two.ts"] },
      }),
      "utf-8",
    );
    fs.writeFileSync(
  ...
  it("derives unscoped ids for scoped packages", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        ...
        mrhammadclaw: { extensions: ["./src/index.ts"] },
      }),
      "utf-8",
    );
    fs.writeFileSync(
  ...
  it("treats configured directory paths as plugin packages", async () => {
    ...
    const { candidates } = await withStateDir(stateDir, async () => {
      return discoverMrHammadClawPlugins({ extraPaths: [packDir] });
    });
  ...
  it("rejects package extension entries that escape via symlink", async () => {
    ...
    fs.writeFileSync(
      path.join(globalExt, "package.json"),
      JSON.stringify({
        name: "@mrhammadclaw/pack",
        mrhammadclaw: { extensions: ["./linked/escape.ts"] },
      }),
      "utf-8",
    );
  ...
  it("rejects package extension entries that are hardlinked aliases", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        name: "@mrhammadclaw/pack",
        mrhammadclaw: { extensions: ["./escape.ts"] },
      }),
      "utf-8",
    );
  ...
  it("ignores package manifests that are hardlinked aliases", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        name: "@mrhammadclaw/pack",
        mrhammadclaw: { extensions: ["./entry.ts"] },
      }),
      "utf-8",
    );
    try {
  ...
  it.runIf(process.platform !== "win32" && typeof process.getuid === "function")(
    ...
    async () => {
      ...
      const result = await withStateDir(stateDir, async () => {
        return discoverMrHammadClawPlugins({ ownershipUid: actualUid + 1 });
      });
      expect(result.candidates).toHaveLength(0);
