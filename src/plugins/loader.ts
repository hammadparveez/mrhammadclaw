...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: MrHammadClawPluginDefinition;
  register?: MrHammadClawPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as MrHammadClawPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as MrHammadClawPluginDefinition;
...
export function loadMrHammadClawPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  const discovery = discoverMrHammadClawPlugins({
    workspaceDir: options.workspaceDir,
  ...
  const getJiti = () => {
    ...
    jitiLoader = createJiti(import.meta.url, {
      ...
      ...(pluginSdkAlias || pluginSdkAccountIdAlias
        ? {
            alias: {
              ...(pluginSdkAlias ? { "mrhammadclaw/plugin-sdk": pluginSdkAlias } : {}),
              ...(pluginSdkAccountIdAlias
                ? { "mrhammadclaw/plugin-sdk/account-id": pluginSdkAccountIdAlias }
                : {}),
            },
          }
        : {}),
  ...
  for (const candidate of discovery.candidates) {
    ...
    let mod: MrHammadClawPluginModule | null = null;
    try {
      mod = getJiti()(safeSource) as MrHammadClawPluginModule;
    } catch (err) {
      recordPluginError({
        logger,
        registry,
