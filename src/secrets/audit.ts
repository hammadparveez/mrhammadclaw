...
import { resolveStateDir, type MrHammadClawConfig } from "../config/config.js";
...
function collectAuthStorePaths(config: MrHammadClawConfig, stateDir: string): string[] {
...
export async function runSecretsAudit(
  ...
  const previousAuthStoreReadOnly = process.env.MRHAMMADCLAW_AUTH_STORE_READONLY;
  process.env.MRHAMMADCLAW_AUTH_STORE_READONLY = "1";
  try {
    ...
    const config = snapshot.valid ? snapshot.config : ({} as MrHammadClawConfig);
    ...
    if (previousAuthStoreReadOnly === undefined) {
      delete process.env.MRHAMMADCLAW_AUTH_STORE_READONLY;
    } else {
      process.env.MRHAMMADCLAW_AUTH_STORE_READONLY = previousAuthStoreReadOnly;
    }
  }
}
