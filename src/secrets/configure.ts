...
function getSecretProviders(config: MrHammadClawConfig): Record<string, SecretProviderConfig> {
...
function removeSecretProvider(config: MrHammadClawConfig, providerAlias: string): boolean {
  if (!isRecord(config.secrets?.providers)) {
    return false;
  }
...
function buildCandidates(config: MrHammadClawConfig): ConfigureCandidate[] {
...
async function configureProvidersInteractive(config: MrHammadClawConfig): Promise<void> {
  while (true) {
...
export async function runSecretsConfigureInteractive(
  ...
  if (!params.providersOnly) {
    ...
    if (candidates.length === 0) {
      throw new Error("No configurable secret-bearing fields found in mrhammadclaw.json.");
    }
  ...
  const plan: SecretsApplyPlan = {
    ...
    generatedBy: "mrhammadclaw secrets configure",
