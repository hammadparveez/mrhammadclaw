...
import type { MrHammadClawConfig } from "../config/types.js";
...
export const DEFAULT_LOG_DIR = resolvePreferredMrHammadClawTmpDir();
export const DEFAULT_LOG_FILE = path.join(DEFAULT_LOG_DIR, "mrhammadclaw.log"); // legacy single-file path

const LOG_PREFIX = "mrhammadclaw";
...
function resolveSettings(): ResolvedSettings {
  let cfg: MrHammadClawConfig["logging"] | undefined =
    (loggingState.overrideSettings as LoggerSettings | null) ?? readLoggingConfig();
  if (!cfg) {
    try {
      const loaded = requireConfig?.("../config/config.js") as
        | {
            loadConfig?: () => MrHammadClawConfig;
          }
        | undefined;
  ...
  const defaultLevel =
    process.env.VITEST === "true" && process.env.MRHAMMADCLAW_TEST_FILE_LOG !== "1"
      ? "silent"
      : "info";
      ...
      if (nextBytes > settings.maxFileBytes) {
        if (!warnedAboutSizeCap) {
          ...
          process.stderr.write(
            `[mrhammadclaw] log file size cap reached; suppressing writes file=${settings.file} maxFileBytes=${settings.maxFileBytes}\n`,
          );
        }
        return;
      }
