...
const TEMP_MEDIA_PREFIX = "mrhammadclaw-media-";
...
function createGroqAudioConfig(): MrHammadClawConfig {
  return {
    tools: {
      media: {
        audio: {
...
function createMediaDisabledConfig(): MrHammadClawConfig {
  return {
    tools: {
      media: {
        audio: { enabled: false },
...
function createMediaDisabledConfigWithAllowedMimes(allowedMimes: string[]): MrHammadClawConfig {
  return {
...
async function setupAudioAutoDetectCase(stdout: string): Promise<{
  ...
  const cfg: MrHammadClawConfig = { tools: { media: { audio: {} } } };
...
describe("applyMediaUnderstanding", () => {
  ...
  beforeAll(async () => {
    const baseDir = resolvePreferredMrHammadClawTmpDir();
  ...
  it("skips audio auto-detect when no supported binaries or provider keys are available", async () => {
    ...
    await withMediaAutoDetectEnv(
      {
        PATH: emptyBinDir,
        MRHAMMADCLAW_AGENT_DIR: isolatedAgentDir,
