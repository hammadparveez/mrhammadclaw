...
export async function withTempWorkspace(
  ...
  const tempHome = await createTempHomeEnv("mrhammadclaw-skills-install-home-");
  const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "mrhammadclaw-skills-install-"));
  try {
