...
describe("resolveDefaultAgentWorkspaceDir", () => {
  it("uses MRHAMMADCLAW_HOME for default workspace resolution", () => {
    ...
    expect(dir).toBe(
      path.join(path.resolve("/srv/mrhammadclaw-home"), ".mrhammadclaw", "workspace"),
    );
  });
});

const WORKSPACE_STATE_PATH_SEGMENTS = [".mrhammadclaw", "workspace-state.json"] as const;
...
describe("ensureAgentWorkspace", () => {
  it("creates BOOTSTRAP.md and records a seeded marker for brand new workspaces", async () => {
    const tempDir = await makeTempWorkspace("mrhammadclaw-workspace-");
    ...
    await expect(fs.access(path.join(tempDir, DEFAULT_MEMORY_DIRNAME))).resolves.toBeUndefined();
...
describe("loadWorkspaceBootstrapFiles", () => {
  ...
  it("treats hardlinked bootstrap aliases as missing", async () => {
    ...
    const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "mrhammadclaw-workspace-hardlink-"));
    try {
