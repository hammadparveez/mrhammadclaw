...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "mrhammadclaw.sandbox=1");
  args.push("--label", `mrhammadclaw.sessionKey=${params.scopeKey}`);
  args.push("--label", `mrhammadclaw.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `mrhammadclaw.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "mrhammadclaw.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`mrhammadclaw sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`mrhammadclaw sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("mrhammadclaw sandbox recreate --all");
}
