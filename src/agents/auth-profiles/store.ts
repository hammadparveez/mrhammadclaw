 ...
 * A common mistake — caused by the similarity with the `mrhammadclaw.json`
...
function normalizeRawCredentialEntry(raw: Record<string, unknown>): Partial<AuthProfileCredential> {
  ...
  // mode → type alias (mrhammadclaw.json uses "mode"; auth-profiles.json uses "type")
...
function loadAuthProfileStoreForAgent(
  ...
  const forceReadOnly = process.env.MRHAMMADCLAW_AUTH_STORE_READONLY === "1";
