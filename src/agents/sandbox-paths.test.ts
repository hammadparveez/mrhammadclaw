...
import { resolvePreferredMrHammadClawTmpDir } from "../infra/tmp-mrhammadclaw-dir.js";
...
async function withOutsideHardlinkInMrHammadClawTmp<T>(
  params: {
...
describe("resolveSandboxedMediaSource", () => {
  const openClawTmpDir = resolvePreferredMrHammadClawTmpDir();

  // Group 1: /tmp paths (the bug fix)
  it.each([
    {
      name: "absolute paths under preferred MrHammadClaw tmp root",
      media: path.join(openClawTmpDir, "image.png"),
      expected: path.join(openClawTmpDir, "image.png"),
    },
    {
      name: "file:// URLs pointing to preferred MrHammadClaw tmp root",
      media: pathToFileURL(path.join(openClawTmpDir, "photo.png")).href,
      expected: path.join(openClawTmpDir, "photo.png"),
    },
    {
      name: "nested paths under preferred MrHammadClaw tmp root",
  ...
  it.each([
    ...
    {
      name: "absolute paths under host tmp outside mrhammadclaw tmp root",
      media: path.join(os.tmpdir(), "outside-mrhammadclaw", "passwd"),
      expected: /sandbox/i,
    },
    {
  ...
  it("rejects symlinked MrHammadClaw tmp paths escaping tmp root", async () => {
  ...
  it("rejects hardlinked MrHammadClaw tmp paths to outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInMrHammadClawTmp(
      {
        openClawTmpDir,
  ...
  it("rejects symlinked MrHammadClaw tmp paths to hardlinked outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInMrHammadClawTmp(
