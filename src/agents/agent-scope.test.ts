...
describe("resolveAgentConfig", () => {
  ...
  it("should return undefined when agent id does not exist", () => {
    const cfg: MrHammadClawConfig = {
      agents: {
        list: [{ id: "main", workspace: "~/mrhammadclaw" }],
      },
    };
  ...
  it("resolves explicit and effective model primary separately", () => {
    ...
    const cfgWithObjectDefault: MrHammadClawConfig = {
      agents: {
        defaults: {
          model: {
    ...
    const cfgNoDefaults: MrHammadClawConfig = {
      agents: {
        list: [{ id: "main" }],
      },
    };
  ...
  it("supports per-agent model primary+fallbacks", () => {
    ...
    const cfgNoOverride: MrHammadClawConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgDisable: MrHammadClawConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgInheritDefaults: MrHammadClawConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-4.1"],
  ...
  it("computes whether any model fallbacks are configured via shared helper", () => {
    const cfgDefaultsOnly: MrHammadClawConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-4.1"],
    ...
    const cfgAgentOverrideOnly: MrHammadClawConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: [],
    ...
    const cfg: MrHammadClawConfig = {
      agents: {
        list: [
          {
            id: "restricted",
            workspace: "~/mrhammadclaw-restricted",
            tools: {
              allow: ["read"],
  ...
  it("should normalize agent id", () => {
    ...
    expect(result?.workspace).toBe("~/mrhammadclaw");
  });

  it("uses MRHAMMADCLAW_HOME for default agent workspace", () => {
    const home = path.join(path.sep, "srv", "mrhammadclaw-home");
    vi.stubEnv("MRHAMMADCLAW_HOME", home);

    const workspace = resolveAgentWorkspaceDir({} as MrHammadClawConfig, "main");
    expect(workspace).toBe(path.join(path.resolve(home), ".mrhammadclaw", "workspace"));
  });

  it("uses MRHAMMADCLAW_HOME for default agentDir", () => {
    const home = path.join(path.sep, "srv", "mrhammadclaw-home");
    vi.stubEnv("MRHAMMADCLAW_HOME", home);
    // Clear state dir so it falls back to MRHAMMADCLAW_HOME
    vi.stubEnv("MRHAMMADCLAW_STATE_DIR", "");

    const agentDir = resolveAgentDir({} as MrHammadClawConfig, "main");
    expect(agentDir).toBe(
      path.join(path.resolve(home), ".mrhammadclaw", "agents", "main", "agent"),
    );
  });
});
