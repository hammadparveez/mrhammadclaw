...
function requireGatewayTool(agentSessionKey?: string) {
  const tool = createMrHammadClawTools({
...
describe("gateway tool", () => {
  ...
  it("schedules SIGUSR1 restart", async () => {
    ...
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "mrhammadclaw-test-"));

    try {
      await withEnvAsync(
        { MRHAMMADCLAW_STATE_DIR: stateDir, MRHAMMADCLAW_PROFILE: "isolated" },
        async () => {
          const tool = requireGatewayTool();
          ...
          expect(parsed.payload?.doctorHint).toBe(
            "Run: mrhammadclaw --profile isolated doctor --non-interactive",
          );
  ...
  it("passes config.apply through gateway call", async () => {
    ...
    const raw = '{\n  agents: { defaults: { workspace: "~/mrhammadclaw" } }\n}\n';
