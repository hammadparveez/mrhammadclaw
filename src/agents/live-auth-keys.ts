...
const GOOGLE_LIVE_SINGLE_KEY = "MRHAMMADCLAW_LIVE_GEMINI_KEY";
...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "MRHAMMADCLAW_LIVE_ANTHROPIC_KEY",
    listVar: "MRHAMMADCLAW_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "MRHAMMADCLAW_LIVE_OPENAI_KEY",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `MRHAMMADCLAW_LIVE_${base}_KEY`;
