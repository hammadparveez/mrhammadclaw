...
async function createAgentDir(): Promise<string> {
  return await fs.mkdtemp(path.join(os.tmpdir(), "mrhammadclaw-pi-auth-storage-"));
}
...
describe("discoverAuthStorage", () => {
  ...
  it("preserves legacy auth.json when auth store is forced read-only", async () => {
    const agentDir = await createAgentDir();
    const previous = process.env.MRHAMMADCLAW_AUTH_STORE_READONLY;
    ...
    try {
      ...
      if (previous === undefined) {
        ...
        process.env.MRHAMMADCLAW_AUTH_STORE_READONLY = previous;
      }
      await fs.rm(agentDir, { recursive: true, force: true });
    }
  });
