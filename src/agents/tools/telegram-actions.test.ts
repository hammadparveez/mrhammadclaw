...
describe("handleTelegramAction", () => {
  ...
  function telegramConfig(overrides?: Record<string, unknown>): MrHammadClawConfig {
    return {
      channels: {
        telegram: {
          botToken: "tok",
  ...
  it("rejects sticker actions when disabled by default", async () => {
    const cfg = { channels: { telegram: { botToken: "tok" } } } as MrHammadClawConfig;
    await expect(
...
describe("handleTelegramAction per-account gating", () => {
  async function expectAccountStickerSend(cfg: MrHammadClawConfig, accountId = "media") {
    await handleTelegramAction(
