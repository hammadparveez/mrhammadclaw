...
function resolveSkillsLimits(config?: MrHammadClawConfig): ResolvedSkillsLimits {
...
function loadSkillEntries(
  ...
  const extraSkills = mergedExtraDirs.flatMap((dir) => {
    ...
    return loadSkills({
      dir: resolved,
      source: "mrhammadclaw-extra",
    });
  });
  const managedSkills = loadSkills({
    dir: managedSkillsDir,
    source: "mrhammadclaw-managed",
  });
  ...
  const skillEntries: SkillEntry[] = Array.from(merged.values()).map((skill) => {
    ...
    return {
      skill,
      frontmatter,
      metadata: resolveMrHammadClawMetadata(frontmatter),
...
function applySkillsPromptLimits(params: { skills: Skill[]; config?: MrHammadClawConfig }): {
...
function resolveWorkspaceSkillPromptState(
  ...
  const truncationNote = truncated
    ? `⚠️ Skills truncated: included ${skillsForPrompt.length} of ${resolvedSkills.length}. Run \`mrhammadclaw skills check\` to audit.`
    : "";
  const prompt = [
    remoteNote,
    truncationNote,
