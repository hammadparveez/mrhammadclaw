    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.acpxRoot, "mrhammadclaw.plugin.json"),
      },
      {
        id: "helper",
        name: "Helper",
        ...
        manifestPath: path.join(params.helperRoot, "mrhammadclaw.plugin.json"),
      },
    ],
  };
}
...
describe("resolvePluginSkillDirs", () => {
  it("keeps acpx plugin skills when ACP is enabled", async () => {
    const workspaceDir = await tempDirs.make("mrhammadclaw-");
    const acpxRoot = await tempDirs.make("mrhammadclaw-acpx-plugin-");
    const helperRoot = await tempDirs.make("mrhammadclaw-helper-plugin-");
