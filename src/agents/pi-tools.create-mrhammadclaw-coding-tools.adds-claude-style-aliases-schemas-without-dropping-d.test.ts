...
describe("createMrHammadClawCodingTools", () => {
  it("returns image metadata for images and text-only blocks for text files", async () => {
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "mrhammadclaw-read-"));
    try {
      ...
      const contents = "Hello from mrhammadclaw read tool.";
  ...
  it("filters tools by sandbox policy", () => {
    ...
    const tools = createMrHammadClawCodingTools({ sandbox });
