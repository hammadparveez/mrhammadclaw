...
describe("createPiToolsSandboxContext", () => {
  it("provides stable defaults for pi-tools sandbox tests", () => {
    ...
    expect(sandbox.containerName).toBe("mrhammadclaw-sbx-test");
    expect(sandbox.containerWorkdir).toBe("/workspace");
    expect(sandbox.docker.image).toBe("mrhammadclaw-sandbox:bookworm-slim");
    expect(sandbox.docker.containerPrefix).toBe("mrhammadclaw-sbx-");
