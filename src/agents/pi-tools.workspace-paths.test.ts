...
describe("workspace path resolution", () => {
  it("resolves relative read/write/edit paths against workspaceDir even after cwd changes", async () => {
    await withTempDir("mrhammadclaw-ws-", async (workspaceDir) => {
      await withTempDir("mrhammadclaw-cwd-", async (otherDir) => {
        const cwdSpy = vi.spyOn(process, "cwd").mockReturnValue(otherDir);
        try {
          const tools = createMrHammadClawCodingTools({ workspaceDir });
  ...
  it("lets exec workdir override the workspace default", async () => {
    await withTempDir("mrhammadclaw-ws-", async (workspaceDir) => {
      await withTempDir("mrhammadclaw-override-", async (overrideDir) => {
        const tools = createMrHammadClawCodingTools({
          workspaceDir,
          exec: { host: "gateway", ask: "off", security: "full" },
        });
  ...
  it("rejects @-prefixed absolute paths outside workspace when workspaceOnly is enabled", async () => {
    await withTempDir("mrhammadclaw-ws-", async (workspaceDir) => {
      const cfg: MrHammadClawConfig = { tools: { fs: { workspaceOnly: true } } };
      const tools = createMrHammadClawCodingTools({ workspaceDir, config: cfg });
      const { readTool } = expectReadWriteEditTools(tools);

      const outsideAbsolute = path.resolve(
        path.parse(workspaceDir).root,
        "outside-mrhammadclaw.txt",
      );
      await expect(
...
describe("sandboxed workspace paths", () => {
  it("uses sandbox workspace for relative read/write/edit", async () => {
    await withTempDir("mrhammadclaw-sandbox-", async (sandboxDir) => {
      await withTempDir("mrhammadclaw-workspace-", async (workspaceDir) => {
        ...
        const tools = createMrHammadClawCodingTools({ workspaceDir, sandbox });
        const { readTool, writeTool, editTool } = expectReadWriteEditTools(tools);
