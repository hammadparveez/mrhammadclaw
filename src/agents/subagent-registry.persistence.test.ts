...
describe("subagent registry persistence", () => {
  const envSnapshot = captureEnv(["MRHAMMADCLAW_STATE_DIR"]);
  ...
  const writePersistedRegistry = async (
    ...
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "mrhammadclaw-subagent-"));
  ...
  it("uses isolated temp state when MRHAMMADCLAW_STATE_DIR is unset in tests", async () => {
    ...
    expect(registryPath).toContain(path.join(os.tmpdir(), "mrhammadclaw-test-state"));
  });
});
