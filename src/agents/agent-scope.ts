...
type AgentEntry = NonNullable<NonNullable<MrHammadClawConfig["agents"]>["list"]>[number];
...
export function listAgentEntries(cfg: MrHammadClawConfig): AgentEntry[] {
...
export function listAgentIds(cfg: MrHammadClawConfig): string[] {
...
export function resolveDefaultAgentId(cfg: MrHammadClawConfig): string {
  const agents = listAgentEntries(cfg);
  if (agents.length === 0) {
    return DEFAULT_AGENT_ID;
  }
...
function resolveAgentEntry(cfg: MrHammadClawConfig, agentId: string): AgentEntry | undefined {
...
export function resolveRunModelFallbacksOverride(params: {
  cfg: MrHammadClawConfig | undefined;
  agentId?: string | null;
  sessionKey?: string | null;
}): string[] | undefined {
  if (!params.cfg) {
...
export function resolveAgentWorkspaceDir(cfg: MrHammadClawConfig, agentId: string) {
  const id = normalizeAgentId(agentId);
...
export function resolveAgentDir(cfg: MrHammadClawConfig, agentId: string) {
  const id = normalizeAgentId(agentId);
