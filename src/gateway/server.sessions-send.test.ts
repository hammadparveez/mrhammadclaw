...
const { createMrHammadClawTools } = await import("../agents/mrhammadclaw-tools.js");
...
type SessionSendTool = ReturnType<typeof createMrHammadClawTools>[number];
...
function getSessionsSendTool(): SessionSendTool {
  ...
  const tool = createMrHammadClawTools().find((candidate) => candidate.name === "sessions_send");
  if (!tool) {
...
beforeAll(async () => {
  envSnapshot = captureEnv(["MRHAMMADCLAW_GATEWAY_PORT", "MRHAMMADCLAW_GATEWAY_TOKEN"]);
  gatewayPort = await getFreePort();
  testState.gatewayAuth = { mode: "token", token: gatewayToken };
  process.env.MRHAMMADCLAW_GATEWAY_PORT = String(gatewayPort);
  process.env.MRHAMMADCLAW_GATEWAY_TOKEN = gatewayToken;
...
describe("sessions_send label lookup", () => {
  it(
    ...
    async () => {
      ...
      if (!configPath) {
        throw new Error("MRHAMMADCLAW_CONFIG_PATH missing in gateway test environment");
      }
