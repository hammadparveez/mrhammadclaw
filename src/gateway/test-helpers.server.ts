...
const GATEWAY_TEST_ENV_KEYS = [
  "HOME",
  "USERPROFILE",
  "MRHAMMADCLAW_STATE_DIR",
  "MRHAMMADCLAW_CONFIG_PATH",
  "MRHAMMADCLAW_SKIP_BROWSER_CONTROL_SERVER",
  "MRHAMMADCLAW_SKIP_GMAIL_WATCHER",
  "MRHAMMADCLAW_SKIP_CANVAS_HOST",
  "MRHAMMADCLAW_BUNDLED_PLUGINS_DIR",
  "MRHAMMADCLAW_SKIP_CHANNELS",
  "MRHAMMADCLAW_SKIP_PROVIDERS",
  "MRHAMMADCLAW_SKIP_CRON",
  "MRHAMMADCLAW_TEST_MINIMAL_GATEWAY",
] as const;
...
async function setupGatewayTestHome() {
  ...
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "mrhammadclaw-gateway-home-"));
  process.env.HOME = tempHome;
  process.env.USERPROFILE = tempHome;
  process.env.MRHAMMADCLAW_STATE_DIR = path.join(tempHome, ".mrhammadclaw");
  delete process.env.MRHAMMADCLAW_CONFIG_PATH;
}

function applyGatewaySkipEnv() {
  process.env.MRHAMMADCLAW_SKIP_BROWSER_CONTROL_SERVER = "1";
  ...
  process.env.MRHAMMADCLAW_SKIP_PROVIDERS = "1";
  process.env.MRHAMMADCLAW_SKIP_CRON = "1";
  process.env.MRHAMMADCLAW_TEST_MINIMAL_GATEWAY = "1";
  process.env.MRHAMMADCLAW_BUNDLED_PLUGINS_DIR = tempHome
    ? path.join(tempHome, "mrhammadclaw-test-no-bundled-extensions")
    : "mrhammadclaw-test-no-bundled-extensions";
}
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  if (options.uniqueConfigRoot) {
    tempConfigRoot = await fs.mkdtemp(path.join(tempHome, "mrhammadclaw-test-"));
  } else {
    tempConfigRoot = path.join(tempHome, ".mrhammadclaw-test");
...
const CONNECT_CHALLENGE_NONCE_KEY = "__mrhammadclawTestConnectChallengeNonce";
const CONNECT_CHALLENGE_TRACKED_KEY = "__mrhammadclawTestConnectChallengeTracked";
...
export async function startServerWithClient(
  ...
  const envSnapshot = captureEnv(["MRHAMMADCLAW_GATEWAY_TOKEN"]);
  const prev = process.env.MRHAMMADCLAW_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.MRHAMMADCLAW_GATEWAY_TOKEN = fallbackToken;
  }
...
function resolveDefaultTestDeviceIdentityPath(params: {
  ...
  const safe =
    `${params.clientId}-${params.clientMode}-${params.platform}-${params.deviceFamily ?? "none"}-${params.role}`
      ...
      .toLowerCase();
  const suiteRoot = process.env.MRHAMMADCLAW_STATE_DIR ?? process.env.HOME ?? os.tmpdir();
