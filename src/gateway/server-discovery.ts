...
export function formatBonjourInstanceName(displayName: string) {
  ...
  if (/mrhammadclaw/i.test(trimmed)) {
    return trimmed;
  }
  return `${trimmed} (MrHammadClaw)`;
}

export function resolveBonjourCliPath(opts: ResolveBonjourCliPathOptions = {}): string | undefined {
  const env = opts.env ?? process.env;
  const envPath = env.MRHAMMADCLAW_CLI_PATH?.trim();
  if (envPath) {
    return envPath;
  }
  ...
  const binCli = path.join(cwd, "bin", "mrhammadclaw");
  if (isFile(binCli)) {
    return binCli;
  }
...
export async function resolveTailnetDnsHint(opts?: {
  ...
  const env = opts?.env ?? process.env;
  const envRaw = env.MRHAMMADCLAW_TAILNET_DNS?.trim();
