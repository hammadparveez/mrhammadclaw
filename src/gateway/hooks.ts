...
export function resolveHooksConfig(cfg: MrHammadClawConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: MrHammadClawConfig, defaultAgentId: string): Set<string> {
...
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken =
    typeof req.headers["x-mrhammadclaw-token"] === "string"
      ? req.headers["x-mrhammadclaw-token"].trim()
      : "";
  if (headerToken) {
    return headerToken;
  }
