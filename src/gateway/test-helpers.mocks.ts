...
type GetReplyFromConfigFn = (
  ...
  configOverride?: MrHammadClawConfig,
...
const testConfigRoot = {
  value: path.join(os.tmpdir(), `mrhammadclaw-gateway-test-${process.pid}-${crypto.randomUUID()}`),
};

export const setTestConfigRoot = (root: string) => {
  testConfigRoot.value = root;
  process.env.MRHAMMADCLAW_CONFIG_PATH = path.join(root, "mrhammadclaw.json");
};
...
vi.mock("../config/config.js", async () => {
  ...
  const resolveConfigPath = () => path.join(testConfigRoot.value, "mrhammadclaw.json");
  ...
  return {
    ...
    loadConfig: () => {
      ...
      const defaults = {
        ...
        workspace: path.join(os.tmpdir(), "mrhammadclaw-gateway-test"),
        ...fileDefaults,
...
vi.mock("../plugins/loader.js", async () => {
  ...
  return {
    ...actual,
    loadMrHammadClawPlugins: () => pluginRegistryState.registry,
  };
});
