...
let lastCreateMrHammadClawToolsContext: Record<string, unknown> | undefined;
...
vi.mock("../agents/mrhammadclaw-tools.js", () => {
  ...
  const tools = [
    ...
    },
    {
      ...
      execute: async () => ({
        ok: true,
        route: {
          agentTo: lastCreateMrHammadClawToolsContext?.agentTo,
          agentThreadId: lastCreateMrHammadClawToolsContext?.agentThreadId,
        },
      }),
    },
    {
  ...
  return {
    createMrHammadClawTools: (ctx: Record<string, unknown>) => {
      lastCreateMrHammadClawToolsContext = ctx;
      return tools;
    },
  };
});
...
beforeEach(() => {
  ...
  pluginHttpHandlers = [];
  cfg = {};
  lastCreateMrHammadClawToolsContext = undefined;
});
...
describe("POST /tools/invoke", () => {
  ...
  it("propagates message target/thread headers into tools context for sessions_spawn", async () => {
    ...
    const res = await invokeTool({
      ...
      headers: {
        ...gatewayAuthHeaders(),
        "x-mrhammadclaw-message-to": "channel:24514",
        "x-mrhammadclaw-thread-id": "thread-24514",
      },
      tool: "sessions_spawn",
      sessionKey: "main",
    });
