...
import { createMrHammadClawTools } from "../agents/mrhammadclaw-tools.js";
import {
...
export async function handleToolsInvokeHttpRequest(
  ...
  const messageChannel = normalizeMessageChannel(
    getHeader(req, "x-mrhammadclaw-message-channel") ?? "",
  );
  const accountId = getHeader(req, "x-mrhammadclaw-account-id")?.trim() || undefined;
  const agentTo = getHeader(req, "x-mrhammadclaw-message-to")?.trim() || undefined;
  const agentThreadId = getHeader(req, "x-mrhammadclaw-thread-id")?.trim() || undefined;
  ...
  const allTools = createMrHammadClawTools({
