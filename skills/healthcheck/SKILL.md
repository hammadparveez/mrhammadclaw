---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for MrHammadClaw deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, MrHammadClaw cron scheduling for periodic checks, or version status checks on a machine running MrHammadClaw (laptop, workstation, Pi, VPS).
---

# MrHammadClaw Host Hardening

## Overview

Assess and harden the host running MrHammadClaw, then align it to a user-defined risk tolerance without breaking access. Use MrHammadClaw security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim MrHammadClaw changes the host firewall, SSH, or OS updates; it does not.
...
5. MrHammadClaw gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if MrHammadClaw can access sensitive data.
...
### 2) Run MrHammadClaw security audits (read-only)

As part of the default read-only checks, run `mrhammadclaw security audit --deep`. Only offer alternatives if the user requests them:

1. `mrhammadclaw security audit` (faster, non-probing)
2. `mrhammadclaw security audit --json` (structured output)

Offer to apply MrHammadClaw safe defaults (numbered):

1. `mrhammadclaw security audit --fix`

Be explicit that `--fix` only tightens MrHammadClaw defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check MrHammadClaw version/update status (read-only)

As part of the default read-only checks, run `mrhammadclaw update status`.
...
- Credential hygiene notes (location of MrHammadClaw creds, prefer disk encryption)
...
- MrHammadClaw security audit (re-run)
...
After MrHammadClaw install or first hardening pass, run at least one baseline audit and version check:

- `mrhammadclaw security audit`
- `mrhammadclaw security audit --deep`
- `mrhammadclaw update status`

Ongoing monitoring is recommended. Use the MrHammadClaw cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `mrhammadclaw cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `mrhammadclaw update status`
...
Before creating, `mrhammadclaw cron list` and match on exact `name`. If found, `mrhammadclaw cron edit <id> ...`.
If not found, `mrhammadclaw cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `mrhammadclaw update status` (preferred for source checkouts and channels)
2. `npm view mrhammadclaw version` (published npm version)

## MrHammadClaw command accuracy

Use only supported commands and flags:

- `mrhammadclaw security audit [--deep] [--fix] [--json]`
- `mrhammadclaw status` / `mrhammadclaw status --deep`
- `mrhammadclaw health --json`
- `mrhammadclaw update status`
- `mrhammadclaw cron add|list|runs|run`

Do not invent CLI flags or imply MrHammadClaw enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by MrHammadClaw compaction:
