# MrHammadClaw ACP Bridge

This document describes how the MrHammadClaw ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`mrhammadclaw acp` exposes an ACP agent over stdio and forwards prompts to a running
MrHammadClaw Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
- ACP client spawns `mrhammadclaw acp` and speaks ACP messages over stdio.
...
mrhammadclaw acp --reset-session
```
...
`mrhammadclaw acp` resolves the Gateway URL and auth from CLI flags or config:
