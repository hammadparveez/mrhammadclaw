...
export async function handleWhatsAppStart(host: MrHammadClawApp, force: boolean) {
  await startWhatsAppLogin(host, force);
  await loadChannels(host, true);
}

export async function handleWhatsAppWait(host: MrHammadClawApp) {
  await waitWhatsAppLogin(host);
  await loadChannels(host, true);
}

export async function handleWhatsAppLogout(host: MrHammadClawApp) {
  await logoutWhatsApp(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigSave(host: MrHammadClawApp) {
  await saveConfig(host);
  await loadConfig(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigReload(host: MrHammadClawApp) {
  await loadConfig(host);
  await loadChannels(host, true);
}
...
function resolveNostrAccountId(host: MrHammadClawApp): string {
...
function resolveGatewayHttpAuthHeader(host: MrHammadClawApp): string | null {
...
function buildGatewayHttpHeaders(host: MrHammadClawApp): Record<string, string> {
  const authorization = resolveGatewayHttpAuthHeader(host);
  return authorization ? { Authorization: authorization } : {};
}

export function handleNostrProfileEdit(
  host: MrHammadClawApp,
  accountId: string,
...
export function handleNostrProfileCancel(host: MrHammadClawApp) {
...
export function handleNostrProfileToggleAdvanced(host: MrHammadClawApp) {
  const state = host.nostrProfileFormState;
  if (!state) {
    return;
  }
...
export async function handleNostrProfileSave(host: MrHammadClawApp) {
  const state = host.nostrProfileFormState;
...
export async function handleNostrProfileImport(host: MrHammadClawApp) {
  const state = host.nostrProfileFormState;
