...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/mrhammadclaw/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/mrhammadclaw");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/mrhammadclaw/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__MRHAMMADCLAW_CONTROL_UI_BASE_PATH__ = "/mrhammadclaw";
    const app = mountApp("/mrhammadclaw/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/mrhammadclaw");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/mrhammadclaw/sessions");
  });
  ...
  it("hydrates token from URL params even when settings already set", async () => {
    localStorage.setItem(
      "mrhammadclaw.control.settings.v1",
